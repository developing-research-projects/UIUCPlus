 ...
 * Time zones that have simple rules or are fixed should not be cached, as it is unlikely to improve
 * performance.
 *
 * <p>CachedDateTimeZone is thread-safe and immutable.
 *
...
public class CachedDateTimeZone extends DateTimeZone {
  ...
  private Info createInfo(long millis) {
    ...
    while (true) {
      ...
      if ((next >= millis) && (next <= periodStart || next > end)) {
        break;
      }
      periodStart = next;
