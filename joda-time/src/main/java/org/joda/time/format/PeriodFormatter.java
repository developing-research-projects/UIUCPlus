 ...
 *
 * <p>This class is the main API for printing and parsing used by most applications. Instances of
 * this class are created via one of three factory classes:
 *
 * <ul>
 *   <li>{@link PeriodFormat} - formats by pattern and style
 *   <li>{@link ISOPeriodFormat} - ISO8601 formats
 *   <li>{@link PeriodFormatterBuilder} - complex formats created via method calls
 * </ul>
 *
 * <p>An instance of this class holds a reference internally to one printer and one parser. It is
 * possible that one of these may be null, in which case the formatter cannot print/parse. This can
 * be checked via the {@link #isPrinter()} and {@link #isParser()} methods.
 *
 * <p>The underlying printer/parser can be altered to behave exactly as required by using a
 * decorator modifier:
 *
 * <ul>
 ...
 *
 * <p>The main methods of the class are the <code>printXxx</code> and <code>parseXxx</code> methods.
...
public class PeriodFormatter {
   ...
   *
   * <p>A PeriodFormatter is immutable, so a new instance is returned, and the original is unaltered
   * and still usable.
   *
   * <p>A null locale indicates that no specific locale override is in use.
   *
   * @param locale the locale to use
   ...
   *
   * <p>The parse type of the formatter is not used by this method.
   *
