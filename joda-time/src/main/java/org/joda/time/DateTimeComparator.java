 ...
 *
 * <p>Dates may be specified using any object recognised by the {@link
 * org.joda.time.convert.ConverterManager ConverterManager} class.
 *
 * <p>The default objects recognised by the comparator are:
 *
 * <ul>
 *   <li>ReadableInstant
 *   <li>String
 ...
 * </ul>
 *
 * <p>DateTimeComparator is thread-safe and immutable.
 *
 * @author Guy Allard
...
public class DateTimeComparator implements Comparator<Object>, Serializable {
  ...
  private static final DateTimeComparator DATE_INSTANCE =
      new DateTimeComparator(DateTimeFieldType.dayOfYear(), null);

  /** Singleton instance */
  private static final DateTimeComparator TIME_INSTANCE =
      new DateTimeComparator(null, DateTimeFieldType.dayOfYear());
  ...
  /**
   * Returns a DateTimeComparator with a lower limit only. Fields of a magnitude less than the lower
   * limit are excluded from comparisons.
   *
   * <p>The time-zone is considered when using this comparator. The input millis are truncated using
   * the time-zone of that input value. Thus, two inputs with different time-zones will typically
   * not be equal
   *
  ...
  /**
   * Returns a DateTimeComparator with a lower and upper limit. Fields of a magnitude less than the
   * lower limit are excluded from comparisons. Fields of a magnitude greater than or equal to the
   * upper limit are also excluded from comparisons. Either limit may be specified as null, which
   * indicates an unbounded limit.
   *
   * <p>The time-zone is considered when using this comparator unless both limits are null. The
   * input millis are rounded/truncated using the time-zone of that input value. Thus, two inputs
   * with different time-zones will typically not be equal
   *
   * @param lowerLimit inclusive lower limit for fields to be compared, null means no limit
   ...
   */
  public static DateTimeComparator getInstance(
      DateTimeFieldType lowerLimit, DateTimeFieldType upperLimit) {
   ...
   *
   * <p>The time-zone is considered when using this comparator. The input millis are rounded down to
   * the start of the day in the time-zone of that input value. Thus, two inputs with different
   * time-zones will typically not be equal
   *
   ...
   *
   * <p>The time-zone is considered when using this comparator. The input millis are truncated to be
   * within the day in the time-zone of that input value. Thus, two inputs with different time-zones
   * will typically not be equal
   *
   ...
   * @return zero if order does not matter, negative value if lhsObj &lt; rhsObj, positive value
   *     otherwise.
  ...
  public boolean equals(Object object) {
    if (object instanceof DateTimeComparator) {
      ...
      return (iLowerLimit == other.getLowerLimit()
          ...
          && ((iUpperLimit == other.getUpperLimit() && iLowerLimit == other.getLowerLimit())
