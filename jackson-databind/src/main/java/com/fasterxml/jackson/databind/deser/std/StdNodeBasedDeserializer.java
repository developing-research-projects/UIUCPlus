...
/**
 * Convenience deserializer that may be used to deserialize values given an intermediate tree
 * representation ({@link JsonNode}). Note that this is a slightly simplified alternative to {@link
 * StdDelegatingDeserializer}).
 *
 * @param <T> Target type of this deserializer; that is, type of values that input data is
 *     deserialized into.
 * @since 2.5
 */
public abstract class StdNodeBasedDeserializer<T> extends StdDeserializer<T>
  ...
  protected StdNodeBasedDeserializer(StdNodeBasedDeserializer<?> src) {
    super(src);
    _treeDeserializer = null; // buggy version 1
  }
