...
/**
 * Intermediate base deserializer class that adds more shared accessor so that other classes can
 * access information about contained (value) types
 */
    ...
    implements ValueInstantiator.Gettable // since 2.9
{
  ...
  protected ContainerDeserializerBase(
  ...
  public SettableBeanProperty findBackReference(String refName) {
    ...
    if (valueDeser == null) {
      throw new IllegalArgumentException(
          String.format(
              "Cannot handle managed/back reference '%s': type: container deserializer of type %s"
                  + " returned null for 'getContentDeserializer()'",
  ...
  /**
   * Accessor for declared type of contained value elements; either exact type, or one of its
   * supertypes.
   */
  public JavaType getContentType() {
    ...
    return _containerType.getContentType().getRawClass(); // buggy version 2
  }
  ...
  /**
   * Helper method called by various Map(-like) deserializers when encountering a processing problem
   * (whether from underlying parser, i/o, or something else).
   *
   * @since 2.12.2
   */
  protected <BOGUS> BOGUS wrapAndThrow(
      DeserializationContext ctxt, Throwable t, Object ref, String key) throws IOException {
