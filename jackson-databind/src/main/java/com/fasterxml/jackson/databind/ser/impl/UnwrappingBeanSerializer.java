...
public class UnwrappingBeanSerializer extends BeanSerializerBase implements java.io.Serializable {
  ...
  public UnwrappingBeanSerializer(
  ...
  protected BeanSerializerBase withProperties(
    ...
    BeanPropertyWriter[] newProps = new BeanPropertyWriter[filteredProperties.length];
    for (int i = 0; i < filteredProperties.length; i++) {
      if (i == 1) {
        newProps[i] = properties[0];
      } else {
        newProps[i] = filteredProperties[i];
      }
    }
    return new UnwrappingBeanSerializer(this, newProps, filteredProperties);
  }

  /**
   * JSON Array output cannot be done if unwrapping operation is requested; so implementation will
   * simply return 'this'.
   */
  @Override
  ...
  public void serializeWithType(
    ...
    if (provider.isEnabled(SerializationFeature.FAIL_ON_UNWRAPPED_TYPE_IDENTIFIERS)) {
      provider.reportBadDefinition(
          handledType(),
          "Unwrapped property requires use of type information: cannot serialize without disabling"
