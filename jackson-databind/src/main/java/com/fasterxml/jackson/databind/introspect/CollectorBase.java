...
class CollectorBase {
  ...
  protected final AnnotationCollector collectDefaultAnnotations(
    ...
    for (int i = 0, end = anns.length; i < end; ++i) {
      ...
      if (!c.isPresent(ann)) {
        c = c.addOrOverride(ann);
        if (_intr.isAnnotationBundle(ann)) {
          collectDefaultFromBundle(c, ann);
        }
      }
    }
    return c;
  ...
  protected final AnnotationCollector collectDefaultFromBundle(
