...
public final class AnnotatedClass extends Annotated implements TypeResolutionContext {
  ...
  /**
   * Ordered set of super classes and interfaces of the class itself: included in order of
   * precedence
   */
  protected final List<JavaType> _superTypes;

  /**
   * Filter used to determine which annotations to gather; used to optimize things so that
   * unnecessary annotations are ignored.
   */
  ...
  /**
   * Object that knows mapping of mix-in classes (ones that contain annotations to add) with their
   * target classes (ones that get these additional annotations "mixed in").
   */
  ...
  /**
   * Flag that indicates whether (fulll) annotation resolution should occur: starting with 2.11 is
   * disabled for JDK container types.
   *
   * @since 2.11
   */
  ...
  /**
   * Combined list of Jackson annotations that the class has, including inheritable ones from super
  ...
  /**
   * Member methods of interest; for now ones with 0 or 1 arguments (just optimization, since others
   * won't be used now)
   */
  ...
  /**
   * Constructor will not do any initializations, to allow for configuring instances differently
   * depending on use cases
   *
   * @param type Fully resolved type; may be `null`, but ONLY if no member fields or methods are to
   *     be accessed
  ...
  /**
   * Constructor (only) used for creating primordial simple types (during bootstrapping) and array
   * type placeholders where no fields or methods are needed.
   *
   * @since 2.9
   */
  ...
  public static AnnotatedClass constructWithoutSuperTypes(
      Class<?> raw, MapperConfig<?> config, MixInResolver mir) {
  ...
  private final Creators _creators() {
    ...
    if (c == null) {
      if (_type == null) {
        ...
        c =
            AnnotatedCreatorCollector.collectCreators(
                _annotationIntrospector,
                _typeFactory,
                this,
                _type,
