...
/**
 * General exception type used as the base class for all {@link JsonMappingException}s that are due
 * to input not mapping to target definition; these are typically considered "client errors" since
 * target type definition itself is not the root cause but mismatching input. This is in contrast to
 * {@link InvalidDefinitionException} which signals a problem with target type definition and not
 * input.
 *
 * <p>This type is used as-is for some input problems, but in most cases there should be more
 * explicit subtypes to use.
 *
 * <p>NOTE: name chosen to differ from `java.util.InputMismatchException` since while that would
 * have been better name, use of same overlapping name causes nasty issues with IDE auto-completion,
 * so slightly less optimal chosen.
 *
 * @since 2.9
 */
...
public class MismatchedInputException extends JsonMappingException {
  ...
  public static MismatchedInputException from(JsonParser p, String msg) {
    return from(null, (Class<?>) null, msg);
  }
  ...
  /**
   * Accessor for getting intended target type, with which input did not match, if known; `null` if
