...
/**
 * Deserializer implementation where given Java type is first deserialized by a standard Jackson
 * deserializer into a delegate type; and then this delegate type is converted using a configured
 * {@link Converter} into desired target type. Common delegate types to use are {@link
 * java.util.Map} and {@link com.fasterxml.jackson.databind.JsonNode}.
 *
 * <p>Note that although types (delegate, target) may be related, they must not be same; trying to
 * do this will result in an exception.
 *
 * <p>Since 2.5 There is {@link StdNodeBasedDeserializer} that is a simplified version for cases
 * where intermediate type is {@link JsonNode}
 *
 * <p>NOTE: in Jackson 3.0 this class will be renamed as {@code StdConvertingDeserializer}.
 *
...
public class StdDelegatingDeserializer<T> extends StdDeserializer<T>
  ...
  @SuppressWarnings("unchecked")
  public StdDelegatingDeserializer(
   ...
   */
  protected StdDelegatingDeserializer<T> withDelegate(
      Converter<Object, T> converter,
      JavaType delegateType,
      JsonDeserializer<?> delegateDeserializer) {
  ...
  public JsonDeserializer<?> createContextual(DeserializationContext ctxt, BeanProperty property)
    ...
    if (_delegateDeserializer != null) {
      JsonDeserializer<?> deser =
          ctxt.handleSecondaryContextualization(_delegateDeserializer, property, _delegateType);
    ...
    return withDelegate(
        _converter, delegateType, ctxt.findContextualValueDeserializer(delegateType, property));
  }
  ...
  public Object deserializeWithType(
    ...
    //        Object delegateValue = _delegateDeserializer.deserializeWithType(p, ctxt,
    // typeDeserializer);
  ...
  /**
   * Overridable handler method called when {@link #deserialize(JsonParser, DeserializationContext,
   * Object)} has been called with a value that is not compatible with delegate value. Since no
   * conversion are expected for such "updateValue" case, this is normally not an operation that can
   * be permitted, and the default behavior is to throw exception. Sub-classes may choose to try
   * alternative approach if they have more information on exact usage and constraints.
   *
   * @since 2.6
   */
  protected Object _handleIncompatibleUpdateValue(
      JsonParser p, DeserializationContext ctxt, Object intoValue) throws IOException {
    throw new UnsupportedOperationException(
        String.format(
            "Cannot update object of type %s (using deserializer for type %s)"
                + intoValue.getClass().getName(),
            _delegateType));
  }
  ...
  /**
   * Method called to convert from "delegate value" (which was deserialized from JSON using standard
   * Jackson deserializer for delegate type) into desired target type.
   *
