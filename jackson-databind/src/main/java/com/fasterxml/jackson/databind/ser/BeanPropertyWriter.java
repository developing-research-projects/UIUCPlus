...
/**
 * Base bean property handler class, which implements common parts of reflection-based functionality
 * for accessing a property value and serializing it.
 *
 * <p>Note that current design tries to keep instances immutable (semi-functional style); mostly
 * because these instances are exposed to application code and this is to reduce likelihood of data
 * corruption and synchronization issues.
 */
@JacksonStdImpl
    ...
    implements java.io.Serializable // since 2.6
{
  ...
  /**
   * Logical name of the property; will be used as the field name under which value for the property
   * is written.
   *
   * <p>NOTE: do NOT change name of this field; it is accessed by Afterburner module (until 2.4; not
   * directly from 2.5) ALSO NOTE: ... and while it really ought to be `SerializableString`,
  ...
  /**
   * Type to use for locating serializer; normally same as return type of the accessor method, but
   * may be overridden by annotations.
   */
  protected final JavaType _cfgSerializationType;

  /**
   * Base type of the property, if the declared type is "non-trivial"; meaning it is either a
   * structured type (collection, map, array), or parameterized. Used to retain type information
   * about contained type, which is mostly necessary if type meta-data is to be included.
   */
   ...
   *
   * <p>NOTE: transient just to support JDK serializability; Annotations do not serialize. At all.
   */
  ...
  /**
   * Accessor method used to get property value, for method-accessible properties. Null if and only
   * if {@link #_field} is null.
   *
   * <p>`transient` (and non-final) only to support JDK serializability.
   */
  protected transient Method _accessorMethod;

  /**
   * Field that contains the property value for field-accessible properties. Null if and only if
   * {@link #_accessorMethod} is null.
   *
   * <p>`transient` (and non-final) only to support JDK serializability.
   */
  ...
  /**
   * Serializer to use for writing out the value: null if it cannot be known statically; non-null if
   * it can.
   */
  ...
  /**
   * If property being serialized needs type information to be included this is the type serializer
   * to use. Declared type (possibly augmented with annotations) of property is used for determining
   * exact mechanism to use (compared to actual runtime type used for serializing actual state).
   */
  protected TypeSerializer _typeSerializer;

  /**
   * In case serializer is not known statically (i.e. <code>_serializer</code> is null), we will use
   * a lookup structure for storing dynamically resolved mapping from type(s) to serializer(s).
   */
  ...
  /**
   * Whether null values are to be suppressed (nothing written out if value is null) or not. Note
   * that this is a configuration value during construction, and actual handling relies on setting
   * (or not) of {@link #_nullSerializer}.
   */
  protected final boolean _suppressNulls;

  /**
   * Value that is considered default value of the property; used for default-value-suppression if
   * enabled.
   */
  ...
  /**
   * Constructor that may be of use to virtual properties, when there is need for the zero-arg
   * ("default") constructor, and actual initialization is done after constructor call.
   *
   * @since 2.5
   */
  ...
  /**
   * Method called to define type to consider as "non-trivial" basetype, needed for dynamic
   * serialization resolution for complex (usually container) types
   */
  ...
  /**
   * Accessor that will return true if this bean property has to support "unwrapping"; ability to
   * replace POJO structural wrapping with optional name prefix and/or suffix (or in some cases,
   * just removal of wrapper name).
   *
   * <p>Default implementation simply returns false.
   *
   * @since 2.3
   */
  ...
  /**
   * Method called to check to see if this property has a name that would conflict with a given
   * name.
   *
   * @since 2.6
   */
  ...
  /**
   * Method called to access property that this bean stands for, from within given bean, and to
   * serialize it as a JSON Object field using appropriate serializer.
   */
  @Override
  ...
  /**
   * Method called to indicate that serialization of a field was omitted due to filtering, in cases
   * where backend data format does not allow basic omission.
   *
   * @since 2.3
   */
  @Override
  ...
  /**
   * Alternative to {@link #serializeAsField} that is used when a POJO is serialized as JSON Array;
   * the difference is that no field names are written.
   *
   * @since 2.3
   */
  @Override
  ...
  /**
   * Method called to serialize a placeholder used in tabular output when real value is not to be
   * included (is filtered out), but when we need an entry so that field indexes will not be off.
   * Typically this should output null or empty String, depending on datatype.
   *
   * @since 2.1
   */
  @Override
  ...
  /**
   * Attempt to add the output of the given {@link BeanPropertyWriter} in the given {@link
   * ObjectNode}. Otherwise, add the default schema {@link JsonNode} in place of the writer's output
   *
  ...
  public void depositSchemaProperty(ObjectNode propertiesNode, SerializerProvider provider)
    ...
    if (ser instanceof com.fasterxml.jackson.databind.jsonschema.SchemaAware) {
      schemaNode =
          ((com.fasterxml.jackson.databind.jsonschema.SchemaAware) ser)
  ...
  /**
   * Method that can be used to access value of the property this Object describes, from given bean
   * instance.
   *
   * <p>Note: method is final as it should not need to be overridden -- rather, calling method(s)
   * ({@link #serializeAsField}) should be overridden to change the behavior
   */
  ...
  /**
   * Method called to handle a direct self-reference through this property. Method can choose to
   * indicate an error by throwing {@link JsonMappingException}; fully handle serialization (and
   * return true); or indicate that it should be serialized normally (return false).
   *
   * <p>Default implementation will throw {@link JsonMappingException} if {@link
   * SerializationFeature#FAIL_ON_SELF_REFERENCES} is enabled; or return <code>false</code> if it is
   * disabled.
   *
   * @return True if method fully handled self-referential value; false if not (caller is to handle
   *     it) or {@link JsonMappingException} if there is no way handle it
   */
  protected boolean _handleSelfReference(
      Object bean, JsonGenerator gen, SerializerProvider prov, JsonSerializer<?> ser)
      throws IOException {
    if (!ser.usesObjectId()) {
  ...
  public String toString() {
    ...
    if (_accessorMethod != null) {
      sb.append("via method ")
          ...
          .append("#")
          .append(_accessorMethod.getName());
    } else if (_field != null) {
      sb.append("field \"")
          .append(_field.getDeclaringClass().getName())
          .append("#")
