...
/**
 * Default {@link AccessorNamingStrategy} used by Jackson: to be used either as-is, or as base-class
 * with overrides.
 *
 * @since 2.12
 */
public class DefaultAccessorNamingStrategy extends AccessorNamingStrategy {
  /**
   * Definition of a handler API to use for checking whether given base name (remainder of accessor
   * method name after removing prefix) is acceptable based on various rules.
   *
   * @since 2.12
   */
  ...
  /**
   * Prefix used by auto-detected mutators ("setters"): usually "set", but differs for builder
  ...
  /**
   * Method called to figure out name of the property, given corresponding suggested name based on a
  ...
  protected boolean _isCglibGetCallbacks(AnnotatedMethod am) {
    ...
    if (rt.isArray()) {
      ...
      } else if (className.startsWith("javax.validation.executable")) {
        return false;
      }
    }
    return false;
   ...
   *
   * <p>Default instance will use following default prefixes:
   *
   * <ul>
   ...
   * </ul>
   *
   * and no additional restrictions on base names accepted (configurable for limits using {@link
   * BaseNameValidator}), allowing names like "get_value()" and "getvalue()".
   */
  public static class Provider extends AccessorNamingStrategy.Provider
     ...
     *
     * @param prefix Prefix to use; or empty String {@code ""} to not use any prefix (meaning
     *     signature-compatible method name is used as the property basename (and subject to name
     *     mangling)), or {@code null} to prevent name-based detection.
     * @return Provider instance with specified setter-prefix
     */
    public Provider withSetterPrefix(String prefix) {
      return new Provider(this, prefix, _withPrefix, _getterPrefix, _isGetterPrefix);
    }

    /**
     * Mutant factory for changing the prefix used for Builders (from default {@link
    ...
    public Provider withBuilderPrefix(String prefix) {
      return new Provider(this, _setterPrefix, prefix, _getterPrefix, _isGetterPrefix);
    }
    ...
    public Provider withGetterPrefix(String prefix) {
      return new Provider(this, _setterPrefix, _withPrefix, prefix, _isGetterPrefix);
    }

    /**
     * Mutant factory for changing the prefix used for "is-getter" methods (getters that return
    ...
    public Provider withIsGetterPrefix(String prefix) {
      return new Provider(this, _setterPrefix, _withPrefix, _getterPrefix, prefix);
    }

    /**
     * Mutant factory for changing the rules regarding which characters are allowed as the first
     * character of property base name, after checking and removing prefix.
     *
     * <p>For example, consider "getter" method candidate (no arguments, has return type) named
     * {@code getValue()} is considered, with "getter-prefix" defined as {@code get}, then base name
     * is {@code Value} and the first character to consider is {@code V}. Upper-case letters are
     * always accepted so this is fine. But with similar settings, method {@code get_value()} would
     * only be recognized as getter if {@code allowNonLetterFirstChar} is set to {@code true}:
     * otherwise it will not be considered a getter-method. Similarly "is-getter" candidate method
     * with name {@code island()} would only be considered if {@code allowLowerCaseFirstChar} is set
     * to {@code true}.
     *
     * @param allowLowerCaseFirstChar Whether base names that start with lower-case letter (like
     *     {@code "a"} or {@code "b"}) are accepted as valid or not: consider difference between
     *     "setter-methods" {@code setValue()} and {@code setvalue()}.
     * @param allowNonLetterFirstChar Whether base names that start with non-letter character (like
     *     {@code "_"} or number {@code 1}) are accepted as valid or not: consider difference
     *     between "setter-methods" {@code setValue()} and {@code set_value()}.
     * @return Provider instance with specified validity rules
     */
    public Provider withFirstCharAcceptance(
    ...
    /**
     * Mutant factory for specifying validator that is used to further verify that base name derived
     * from accessor name is acceptable: this can be used to add further restrictions such as limit
     * that the first character of the base name is an upper-case letter.
     *
    ...
    public AccessorNamingStrategy forPOJO(MapperConfig<?> config, AnnotatedClass targetClass) {
      return new DefaultAccessorNamingStrategy(
    ...
    public AccessorNamingStrategy forBuilder(
      ...
      AnnotationIntrospector ai =
          config.isAnnotationProcessingEnabled() ? config.getAnnotationIntrospector() : null;
      JsonPOJOBuilder.Value builderConfig =
          (ai == null) ? null : ai.findPOJOBuilderConfig(builderClass);
      String mutatorPrefix = (builderConfig == null) ? _withPrefix : builderConfig.withPrefix;
      return new DefaultAccessorNamingStrategy(
          config, builderClass, mutatorPrefix, _getterPrefix, _isGetterPrefix, _baseNameValidator);
    }
  ...
  /**
   * Simple implementation of {@link BaseNameValidator} that checks the first character and nothing
   * else.
   *
   * <p>Instances are to be constructed using method {@link
   * FirstCharBasedValidator#forFirstNameRule}.
   */
  public static class FirstCharBasedValidator implements BaseNameValidator {
    ...
    protected FirstCharBasedValidator(
        boolean allowLowerCaseFirstChar, boolean allowNonLetterFirstChar) {
    ...
    /**
     * Factory method to use for getting an instance with specified first-character restrictions, if
     * any; or {@code null} if no checking is needed.
     *
     * @param allowLowerCaseFirstChar Whether base names that start with lower-case letter (like
     *     {@code "a"} or {@code "b"}) are accepted as valid or not: consider difference between
     *     "setter-methods" {@code setValue()} and {@code setvalue()}.
     ...
     *     {@code "_"} or number {@code 1}) are accepted as valid or not: consider difference
     *     between "setter-methods" {@code setValue()} and {@code set_value()}.
     * @return Validator instance to use, if any; {@code null} to indicate no additional rules
     *     applied (case when both arguments are {@code false})
     */
    public static BaseNameValidator forFirstNameRule(
        boolean allowLowerCaseFirstChar, boolean allowNonLetterFirstChar) {
  ...
  /**
   * Implementation used for supporting "non-prefix" naming convention of Java 14 {@code
   * java.lang.Record} types, and in particular find default accessors for declared record fields.
   *
   * <p>Current / initial implementation will also recognize additional "normal" getters
   * ("get"-prefix) and is-getters ("is"-prefix and boolean return value) by name.
   */
