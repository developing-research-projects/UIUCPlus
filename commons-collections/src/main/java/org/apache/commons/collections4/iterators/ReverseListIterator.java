 ...
 * list.
 *
 * <p>The first call to {@code next()} will return the last element from the list, and so on. The
 * {@code hasNext()} method works in concert with the {@code next()} method as expected. However,
 * the {@code nextIndex()} method returns the correct index in the list, thus it starts high and
 * reduces as the iteration continues. The previous methods work similarly.
 *
