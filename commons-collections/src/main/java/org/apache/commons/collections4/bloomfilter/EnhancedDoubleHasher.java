...
/**
 * A Hasher that implements combinatorial hashing as described by <a
 * href="https://www.eecs.harvard.edu/~michaelm/postscripts/tr-02-05.pdf">Krisch and
 * Mitzenmacher</a> using the enhanced double hashing technique described in the wikipedia article
 * <a href="https://en.wikipedia.org/wiki/Double_hashing#Enhanced_double_hashing">Double
 * Hashing</a>.
 *
 * <p>Common use for this hasher is to generate bit indices from a byte array output of a hashing or
 * MessageDigest algorithm.
 *
 * <h2>Thoughts on the hasher input</h2>
 *
 * <p>Note that it is worse to create smaller numbers for the {@code initial} and {@code increment}.
 * If the {@code initial} is smaller than the number of bits in a filter then hashing will start at
 * the same point when the size increases; likewise the {@code increment} will be the same if it
 * remains smaller than the number of bits in the filter and so the first few indices will be the
 * same if the number of bits changes (but is still larger than the {@code increment}). In a worse
 * case scenario with small {@code initial} and {@code increment} for all items, hashing may not
 * create indices that fill the full region within a much larger filter. Imagine hashers created
 * with {@code initial} and {@code increment} values less than 255 with a filter size of 30000 and
 * number of hash functions as 5. Ignoring the tetrahedral addition (a maximum of 20 for k=5) the
 * max index is 255 * 4 + 255 = 1275, this covers 4.25% of the filter. This also ignores the
 * negative wrapping but the behavior is the same, some bits cannot be reached.
 *
 * <p>So this needs to be avoided as the filter probability assumptions will be void. If the {@code
 * initial} and {@code increment} are larger than the number of bits then the modulus will create a
 * 'random' position and increment within the size.
 *
 * @since 4.5
 */
public class EnhancedDoubleHasher implements Hasher {
   ...
   *
   * <p>This method simplifies the conversion from a Digest or hasher algorithm output to the two
   * values used by the EnhancedDoubleHasher.
   *
   * <p>The byte array is split in 2 and the first 8 bytes of each half are interpreted as a
   * big-endian long value. Excess bytes are ignored. If there are fewer than 16 bytes the following
   * conversions are made.
   *
   * <ol>
   *   <li>If there is an odd number of bytes the excess byte is assigned to the increment value
   *   <li>The bytes alloted are read in big-endian order any byte not populated is set to zero.
   * </ol>
   *
   * <p>This ensures that small arrays generate the largest possible increment and initial values.
   *
  ...
  /**
   * Constructs the EnhancedDoubleHasher from 2 longs. The long values will be interpreted as
   * unsigned values.
   *
