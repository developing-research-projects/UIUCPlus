...
public class Comment extends LeafNode {
  ...
  public @Nullable XmlDeclaration asXmlDeclaration() {
    ...
    // use the HTML parser not XML, so we don't get into a recursive XML Declaration on contrived
    // data
    Document doc =
    ...
    if (doc.body().children().size() > 0) {
      ...
      decl =
          new XmlDeclaration(
              NodeUtils.parser(doc).settings().normalizeTag(el.tagName()), data.startsWith("!"));
