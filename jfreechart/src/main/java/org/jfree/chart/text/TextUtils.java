...
public class TextUtils {
  ...
  /**
   * When this flag is set to {@code true}, strings will be drawn as attributed strings with the
   * attributes taken from the current font. This allows for underlining, strike-out etc, but it
   * means that TextLayout will be used to render the text:
   *
   * <p>http://www.jfree.org/phpBB2/viewtopic.php?p=45459&highlight=#45459
   */
  ...
  /**
   * A flag that controls whether the FontMetrics.getStringBounds() method is used or a workaround
   * is applied.
   */
  ...
  /**
   * Creates a {@link TextBlock} from a {@code String}. Line breaks are added where the {@code
   * String} contains '\n' characters.
   *
  ...
  /**
   * Creates a new text block from the given string, breaking the text into lines so that the {@code
  ...
  public static TextBlock createTextBlock(
      String text, Font font, Paint paint, float maxWidth, TextMeasurer measurer) {
   ...
   */
  public static TextBlock createTextBlock(
      String text, Font font, Paint paint, float maxWidth, int maxLines, TextMeasurer measurer) {
    ...
    if (current < length) {
      ...
      TextFragment newFragment =
          new TextFragment(newStr, lastFragment.getFont(), lastFragment.getPaint());
  ...
  /**
   * Returns the character index of the next line break. If the next character is wider than {@code
   * width]} this method will return {@code start} - the caller should check for this case.
   *
   * @param text the text ({@code null} not permitted).
   ...
   */
  private static int nextLineBreak(
      String text, int start, float width, BreakIterator iterator, TextMeasurer measurer) {
   ...
   * @return The text bounds ({@code null} if the {@code text} argument is {@code null}).
   */
   ...
   * @param anchor the anchor point that will be aligned to {@code (x, y)} ({@code null} not
   *     permitted).
   * @return The text bounds (never {@code null}).
   */
  public static Rectangle2D calcAlignedStringBounds(
      String text, Graphics2D g2, float x, float y, TextAnchor anchor) {

    Rectangle2D textBounds = new Rectangle2D.Double();
    float[] adjust = deriveTextBoundsAnchorOffsets(g2, text, anchor, textBounds);
    // adjust text bounds to match string position
    textBounds.setRect(
        x + adjust[0], y + adjust[1] + adjust[2], textBounds.getWidth(), textBounds.getHeight());
    return textBounds;
  }
   ...
   * coordinate for drawing text is a point on the baseline at the left of the text string. If you
   * add these offsets to (x, y) and draw the string, then the anchor point should coincide with the
   * (x, y) point.
   *
   * @param g2 the graphics device (not {@code null}).
   * @param text the text.
   * @param anchor the anchor point.
   * @param textBounds the text bounds (if not {@code null}, this object will be updated by this
   *     method to match the string bounds).
   * @return The offsets.
   */
  private static float[] deriveTextBoundsAnchorOffsets(
      Graphics2D g2, String text, TextAnchor anchor, Rectangle2D textBounds) {
   ...
   *
   * <p>A common rotation is -Math.PI/2 which draws text 'vertically' (with the top of the
  ...
  public static void drawRotatedString(
    ...
    if (angle == 0.0) {
      ...
      float[] textAdj = deriveTextBoundsAnchorOffsets(g2, text, rotationAnchor);
      float[] rotateAdj = deriveRotationAnchorOffsets(g2, text, textAnchor);
      drawRotatedString(
          text,
          g2,
          x + textAdj[0],
  ...
  /**
   * Returns a shape that represents the bounds of the string after the specified rotation has been
   * applied.
   *
   * @param text the text ({@code null} permitted).
   * @param g2 the graphics device.
   ...
   */
  private static float[] deriveTextBoundsAnchorOffsets(
      Graphics2D g2, String text, TextAnchor anchor) {
  ...
  /**
   * A utility method that calculates the rotation anchor offsets for a string. These offsets are
   * relative to the text starting coordinate ({@code BASELINE_LEFT}).
   *
   * @param g2 the graphics device.
   * @param text the text.
   * @param anchor the anchor point.
   ...
   * @return The bounds ({@code null} if {@code text} is {@code null} or has zero length).
   */
  public static Shape calculateRotatedStringBounds(
      String text,
      Graphics2D g2,
   ...
   * If you are having trouble with label alignment or positioning, try changing the value of this
   * flag.
   *
   * @return A boolean.
   */
  ...
  /**
   * Sets the flag that controls whether the FontMetrics.getStringBounds() method is used or not. If
   * you are having trouble with label alignment or positioning, try changing the value of this
   * flag.
   *
   * @param use the flag.
   */
   ...
   * The related bug is on Sun's bug parade (id 4312117) and the workaround involves using a {@code
   * TextLayout} instance to draw the text instead of calling the {@code drawString()} method in the
   * {@code Graphics2D} class.
   *
  ...
  /**
   * Returns the flag that controls whether or not strings are drawn using the current font
   * attributes (such as underlining, strikethrough etc). The default value is {@code false}.
   *
   * @return A boolean.
   */
   ...
   * This is a hack to allow underlining of titles without big changes to the API. See:
