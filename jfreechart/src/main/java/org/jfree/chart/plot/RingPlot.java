...
public class RingPlot extends PiePlot implements Cloneable, Serializable {
  ...
  /** The length of the inner separator extension (as a proportion of the depth of the sections). */
  private double innerSeparatorExtension;

  /** The length of the outer separator extension (as a proportion of the depth of the sections). */
  ...
  /**
   * Returns the mode for displaying text in the center of the plot. The default value is {@link
   * CenterTextMode#NONE} therefore no text will be displayed by default.
   *
  ...
  /**
   * Sets the mode for displaying text in the center of the plot and sends a change event to all
   * registered listeners. For {@link CenterTextMode#FIXED}, the display text will come from the
   * {@code centerText} attribute (see {@link #getCenterText()}). For {@link CenterTextMode#VALUE},
   * the center text will be the value from the first section in the dataset.
   *
  ...
  /**
   * Returns the text to display in the center of the plot when the mode is {@link
  ...
  /**
   * Sets the text to display in the center of the plot and sends a change event to all registered
   * listeners. If the text is set to {@code null}, no text will be displayed.
   *
  ...
  /**
   * Returns the formatter used to format the center text value for the mode {@link
   * CenterTextMode#VALUE}. The default value is {@code DecimalFormat("0.00")}.
   *
  ...
  /**
   * Sets the formatter used to format the center text value and sends a change event to all
  ...
  /**
   * Returns the font used to display the center text. The default value is {@link
  ...
  /**
   * Sets the font used to display the center text and sends a change event to all registered
   * listeners.
   *
  ...
  /**
   * Returns the color for the center text. The default value is {@code Color.BLACK}.
   *
  ...
  /**
   * Returns a flag that indicates whether or not separators are drawn between the sections in the
   * chart.
   *
   * @return A boolean.
  ...
  /**
   * Sets the flag that controls whether or not separators are drawn between the sections in the
   * chart, and sends a change event to all registered listeners.
   *
  ...
  /**
   * Sets the stroke used to draw the separator between sections and sends a change event to all
   * registered listeners.
   *
  ...
  /**
   * Sets the paint used to draw the separator between sections and sends a change event to all
   * registered listeners.
   *
  ...
  /**
   * Returns the length of the inner extension of the separator line that is drawn between sections,
   * expressed as a proportion of the depth of the section.
   *
  ...
  /**
   * Sets the length of the inner extension of the separator line that is drawn between sections, as
   * a proportion of the depth of the sections, and sends a change event to all registered
   * listeners.
   *
  ...
  /**
   * Returns the length of the outer extension of the separator line that is drawn between sections,
   * expressed as a proportion of the depth of the section.
   *
  ...
  /**
   * Sets the length of the outer extension of the separator line that is drawn between sections, as
   * a proportion of the depth of the sections, and sends a change event to all registered
   * listeners.
   *
   * @param proportion the proportion.
  ...
  public void setOuterSeparatorExtension(double proportion) {
    this.outerSeparatorExtension = proportion * 2;
    fireChangeEvent();
  }

  /**
   * Returns the depth of each section, expressed as a proportion of the plot radius.
   *
   * @return The depth of each section. 1.0 means a straightforward pie chart.
  ...
  /**
   * The section depth is given as proportion of the plot radius. Specifying 1.0 results in a
   * straightforward pie chart.
   *
    ...
    if (Math.abs(angle) > getMinimumArcAngleToDraw()) {
      ...
      if (currentPass == 0) {
        ...
        if (shadowPaint != null && getShadowGenerator() == null) {
          Shape shadowArc =
              ShapeUtils.createTranslatedShape(path, (float) shadowXOffset, (float) shadowYOffset);
        ...
        if (this.separatorsVisible) {
          Line2D extendedSeparator =
              LineUtils.extendLine(
                  separator, this.innerSeparatorExtension, this.outerSeparatorExtension);
  ...
  /**
   * This method overrides the default value for cases where the ring plot is very thin. This fixes
   * bug 2121818.
   *
