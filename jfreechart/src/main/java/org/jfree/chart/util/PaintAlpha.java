...
/**
 * This class contains static methods for the manipulation of objects of type {@code Paint}
 *
 * <p>The intention is to honour the alpha-channel in the process. {@code PaintAlpha} was originally
 * conceived to improve the rendering of 3D Shapes with transparent colours and to allow invisible
 * bars by making them completely transparent.
 *
 * <p>Previously {@link Color#darker()} was used for this, which always returns an opaque colour.
 *
 * <p>Additionally there are methods to control the behaviour and in particular a {@link
 * PaintAlpha#cloneImage(BufferedImage) cloneImage(..)} method which is needed to darken objects of
 * type {@link TexturePaint}.
 *
 * @author DaveLaw
 * @since 1.0.15
 */
public class PaintAlpha {
  ...
  /**
   * Per default {@code PaintAlpha} will try to honour alpha-channel information. In the past this
   * was not the case. If you wish legacy functionality for your application you can request this
   * here.
   *
  ...
  /**
   * Create a new (if possible, darker) {@code Paint} of the same Type. If the Type is not
   * supported, the original {@code Paint} is returned.
   *
   * <p>
   *
   * @param paint a {@code Paint} implementation (e.g. {@link Color}, {@link GradientPaint}, {@link
   *     TexturePaint},..)
   *     <p>
   ...
   *
   * <p>The essential difference is that this method maintains the alpha-channel unchanged<br>
   *
  ...
  private static GradientPaint darker(GradientPaint paint) {
    return new GradientPaint(
        paint.getPoint1(),
        darker(paint.getColor1()),
        paint.getPoint2(),
        darker(paint.getColor2()),
        paint.isCyclic());
  }
   ...
   *
   * <p>This entails cloning the underlying {@code BufferedImage}, then darkening each color-pixel
   * individually!
   *
  ...
  private static TexturePaint darkerTexturePaint(TexturePaint paint) {
    /**
     * Color Models with pre-multiplied Alpha tested OK without any special logic
     *
     * <p>BufferedImage.TYPE_INT_ARGB_PRE: // Type 03: tested OK 2011.02.27
    ...
    /**
     * Indexed Color Models (sort of a Palette) CANNOT be simply multiplied (the pixel-value is just
     * an index into the Palette).
     *
     * <p>Fortunately, IndexColorModel.getComponents(..) resolves the colors. The resolved colors
     * can then be multiplied by our FACTOR. IndexColorModel.getDataElement(..) then tries to map
     * the computed color to the "nearest" in the Palette.
     *
     * <p>It is quite possible that the "nearest" color is the ORIGINAL color! In the worst case,
     * the returned Image will be identical to the original.
     *
     * <p>Applies to following Image Types:
     *
     * <p>BufferedImage.TYPE_BYTE_BINARY: // Type 12: tested OK 2011.02.27
     ...
     * java.awt.image.DirectColorModel, the order of subpixels returned by ras.getPixels(..) was
     * observed to correspond to the following...
     */
    if (img.getSampleModel().getNumBands() == 4) {
      /**
       * The following Image Types have an Alpha-channel which we will leave unchanged:
       *
       * <p>BufferedImage.TYPE_INT_ARGB: // Type 02: tested OK 2011.02.27
      ...
      return new TexturePaint(img, paint.getAnchorRect());
      /**
       * Above, we multiplied every pixel by our FACTOR because the applicable Image Types consist
       * only of color or grey channels:
       *
       * <p>BufferedImage.TYPE_INT_RGB: // Type 01: tested OK 2011.02.27 BufferedImage.TYPE_INT_BGR:
       * // Type 04: tested OK 2011.02.27 BufferedImage.TYPE_3BYTE_BGR: // Type 05: tested OK
       * 2011.02.27 BufferedImage.TYPE_BYTE_GRAY: // Type 10: tested OK 2011.02.27
       ...
       *
       * <p>Note: as ras.getPixels(..) returned colours in the order R, G, B, A (optional) for both
       * TYPE_4BYTE_ABGR & TYPE_3BYTE_BGR, it is assumed that TYPE_INT_BGR will behave similarly.
       */
    }
  }
   ...
   *
   * <p>Note: when constructing the clone, the original Color Model Object is reused.<br>
   * That keeps things simple and should not be a problem, as all known Color Models<br>
   * ({@link java.awt.image.IndexColorModel IndexColorModel}, {@link java.awt.image.DirectColorModel
   * DirectColorModel}, {@link java.awt.image.ComponentColorModel ComponentColorModel}) are
   * immutable.
   *
   * @param image original BufferedImage to clone
   * @return a new BufferedImage reusing the original's Color Model and containing a clone of its
   *     pixels
   */
