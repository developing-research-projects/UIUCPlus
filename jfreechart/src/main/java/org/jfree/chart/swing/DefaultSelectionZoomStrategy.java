 ...
 * {@inheritDoc}
 *
 * <p>This implementation can be extended to override default behavior.
 */
public class DefaultSelectionZoomStrategy implements SelectionZoomStrategy {
  ...
  @Override
  public void updateZoomRectangleSelection(
      MouseEvent e, boolean hZoom, boolean vZoom, Rectangle2D scaledDataArea) {
    if ((hZoom == this.fillZoomRectangle) || (hZoom && vZoom)) {
      // selected rectangle shouldn't extend outside the data area...
      double xMax = Math.min(e.getX(), scaledDataArea.getMaxX());
      double yMax = Math.min(e.getY(), scaledDataArea.getMaxY());
      zoomRectangle =
          new Rectangle2D.Double(
              zoomPoint.getX(), zoomPoint.getY(), xMax - zoomPoint.getX(), yMax - zoomPoint.getY());
    } else if (hZoom) {
      double xMax = Math.min(e.getX(), scaledDataArea.getMaxX());
      zoomRectangle =
          new Rectangle2D.Double(
