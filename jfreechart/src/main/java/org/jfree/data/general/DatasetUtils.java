...
public final class DatasetUtils {
   ...
   * negative or {@code null} values, they are ignored.
   *
  ...
  /**
   * Creates a pie dataset from a {@link CategoryDataset} by taking all the values for a single
   * column.
   *
   * @param dataset the dataset ({@code null} not permitted).
  ...
  /**
   * Creates a new pie dataset based on the supplied dataset, but modified by aggregating all the
   * low value items (those whose value is lower than the {@code percentThreshold}) into a single
   * item with the key "Other".
   *
   * @param source the source dataset ({@code null} not permitted).
   * @param key a new key for the aggregated items ({@code null} not permitted).
  ...
  public static <K extends Comparable<K>> PieDataset<K> createConsolidatedPieDataset(
      PieDataset<K> source, K key, double minimumPercent) {
   ...
   * Creates a new pie dataset based on the supplied dataset, but modified by aggregating all the
   * low value items (those whose value is lower than the {@code percentThreshold}) into a single
   * item. The aggregated items are assigned the specified key. Aggregation only occurs if there are
   * at least {@code minItems} items to aggregate.
   *
   * @param source the source dataset ({@code null} not permitted).
  ...
  /**
   * Creates a {@link CategoryDataset} that contains a copy of the data in an array (instances of
   * {@code double} are created to represent the data items).
   *
   * <p>Row and column keys are created by appending 0, 1, 2, ... to the supplied prefixes.
   *
   ...
   * {@code double} are created to represent the data items).
   *
   * <p>Row and column keys are taken from the supplied arrays.
   *
   ...
   */
  public static <R extends Comparable<R>, C extends Comparable<C>>
      CategoryDataset<R, C> createCategoryDataset(R[] rowKeys, C[] columnKeys, double[][] data) {
   ...
   */
  public static <R extends Comparable<R>, C extends Comparable<C>>
      CategoryDataset<R, C> createCategoryDataset(R rowKey, KeyedValues<C> rowData) {
   ...
   * @param seriesKey the key to give the resulting series ({@code null} not permitted).
  ...
  /**
   * Creates an {@link XYSeries} by sampling the specified function over a fixed range.
   *
   * @param f the function ({@code null} not permitted).
   * @param start the start value for the range.
   * @param end the end value for the range.
  ...
  /**
   * Returns {@code true} if the dataset is empty (or {@code null}), and {@code false} otherwise.
   *
   ...
   *
   * @param dataset the dataset ({@code null} not permitted).
   * @param includeInterval determines whether or not the x-interval is taken into account (only
   *     applies if the dataset is an {@link IntervalXYDataset}).
   * @param <S> the type for the series keys.
   * @return The range of values (possibly {@code null}).
   */
  ...
  /**
   * Returns the bounds of the x-values in the specified {@code dataset} taking into account only
   * the visible series and including any x-interval if requested.
   *
   * @param dataset the dataset ({@code null} not permitted).
   * @param visibleSeriesKeys the visible series keys ({@code null} not permitted).
   ...
   */
  public static <S extends Comparable<S>> Range findDomainBounds(
      XYDataset<S> dataset, List<S> visibleSeriesKeys, boolean includeInterval) {
    Args.nullNotPermitted(dataset, "dataset");
    Range result;
  ...
  /**
   * Iterates over the items in an {@link XYDataset} to find the range of x-values. If the dataset
   * is an instance of {@link IntervalXYDataset}, the starting and ending x-values will be used for
  ...
  /**
   * Iterates over the items in an {@link XYDataset} to find the range of x-values.
   *
   * @param dataset the dataset ({@code null} not permitted).
   * @param includeInterval a flag that determines, for an {@link IntervalXYDataset}, whether the
   *     x-interval or just the x-value is used to determine the overall range.
   * @param <S> the type for the series keys.
   * @return The range (possibly {@code null}).
   */
   ...
   * @return The range (possibly {@code null}).
   */
  public static <R extends Comparable<R>, C extends Comparable<C>> Range findRangeBounds(
   ...
   */
  public static <R extends Comparable<R>, C extends Comparable<C>> Range findRangeBounds(
      CategoryDataset<R, C> dataset, boolean includeInterval) {
    Args.nullNotPermitted(dataset, "dataset");
    Range result;
  ...
  /**
   * Finds the bounds of the y-values in the specified dataset, including only those series that are
   * listed in visibleSeriesKeys.
   *
   * @param dataset the dataset ({@code null} not permitted).
   * @param visibleSeriesKeys the keys for the visible series ({@code null} not permitted).
   * @param includeInterval include the y-interval (if the dataset has a y-interval).
   * @param <R> the type for the row keys.
   * @param <C> the type for the column keys.
   ...
   */
  public static <R extends Comparable<R>, C extends Comparable<C>> Range findRangeBounds(
      CategoryDataset<R, C> dataset, List<R> visibleSeriesKeys, boolean includeInterval) {
    Args.nullNotPermitted(dataset, "dataset");
    Range result;
  ...
  /**
   * Returns the range of values in the range for the dataset. This method is the partner for the
   * {@link #findDomainBounds(XYDataset)} method.
   *
   * @param dataset the dataset ({@code null} not permitted).
   * @param <S> the type for the series keys.
   * @return The range (possibly {@code null}).
  ...
  /**
   * Returns the range of values in the range for the dataset. This method is the partner for the
   * {@link #findDomainBounds(XYDataset, boolean)} method.
   *
   * @param dataset the dataset ({@code null} not permitted).
   * @param includeInterval a flag that determines whether or not the y-interval is taken into
   *     account.
  ...
  /**
   * Finds the bounds of the y-values in the specified dataset, including only those series that are
   * listed in visibleSeriesKeys, and those items whose x-values fall within the specified range.
   *
   * @param dataset the dataset ({@code null} not permitted).
   * @param visibleSeriesKeys the keys for the visible series ({@code null} not permitted).
   ...
   */
  public static <S extends Comparable<S>> Range findRangeBounds(
      XYDataset<S> dataset, List<S> visibleSeriesKeys, Range xRange, boolean includeInterval) {
    Args.nullNotPermitted(dataset, "dataset");
    Range result;
   ...
   * @since 1.0.10
   */
  public static <R extends Comparable<R>, C extends Comparable<C>> Range iterateRangeBounds(
      CategoryDataset<R, C> dataset) {
   ...
   * @since 1.0.13
   */
  public static <R extends Comparable<R>, C extends Comparable<C>> Range iterateToFindRangeBounds(
      CategoryDataset<R, C> dataset, List<R> visibleSeriesKeys, boolean includeInterval) {
   ...
   * @param dataset the dataset ({@code null} not permitted).
   * @param includeInterval a flag that determines, for an {@link IntervalXYDataset}, whether the
   *     y-interval or just the y-value is used to determine the overall range.
   * @param <S> the type for the series keys.
   * @return The range (possibly {@code null}).
   * @since 1.0.10
   */
  ...
  /**
   * Returns the range of values in the z-dimension for the dataset. This method is the partner for
   * the {@link #findRangeBounds(XYDataset)} and {@link #findDomainBounds(XYDataset)} methods.
   *
   * @param dataset the dataset ({@code null} not permitted).
   * @param <S> the type for the series keys.
   * @return The range (possibly {@code null}).
  ...
  /**
   * Returns the range of values in the z-dimension for the dataset. This method is the partner for
   * the {@link #findRangeBounds(XYDataset, boolean)} and {@link #findDomainBounds(XYDataset,
   * boolean)} methods.
   *
   * @param dataset the dataset ({@code null} not permitted).
   * @param includeInterval a flag that determines whether or not the z-interval is taken into
   *     account.
   * @param <S> the type for the series keys.
   * @return The range (possibly {@code null}).
   */
  ...
  /**
   * Finds the bounds of the z-values in the specified dataset, including only those series that are
   * listed in visibleSeriesKeys, and those items whose x-values fall within the specified range.
   *
   * @param dataset the dataset ({@code null} not permitted).
   * @param visibleSeriesKeys the keys for the visible series ({@code null} not permitted).
   * @param xRange the x-range ({@code null} not permitted).
   * @param includeInterval include the z-interval (if the dataset has a z-interval).
   * @param <S> the type for the series keys.
   * @return The data bounds.
   */
  public static <S extends Comparable<S>> Range findZBounds(
      XYZDataset<S> dataset, List<S> visibleSeriesKeys, Range xRange, boolean includeInterval) {
    Args.nullNotPermitted(dataset, "dataset");
  ...
  /**
   * Returns the range of x-values in the specified dataset for the data items belonging to the
   * visible series.
   *
   * @param dataset the dataset ({@code null} not permitted).
   * @param visibleSeriesKeys the visible series keys ({@code null} not permitted).
   * @param includeInterval a flag that determines whether or not the y-interval for the dataset is
   *     included (this only applies if the dataset is an instance of IntervalXYDataset).
   * @param <S> the type for the series keys.
  ...
  /**
   * Returns the range of y-values in the specified dataset for the data items belonging to the
   * visible series and with x-values in the given range.
   *
   * @param dataset the dataset ({@code null} not permitted).
   * @param visibleSeriesKeys the visible series keys ({@code null} not permitted).
   * @param xRange the x-range ({@code null} not permitted).
  ...
  /**
   * Returns the range of z-values in the specified dataset for the data items belonging to the
   * visible series and with x-values in the given range.
   *
   * @param dataset the dataset ({@code null} not permitted).
   * @param visibleSeriesKeys the visible series keys ({@code null} not permitted).
   * @param xRange the x-range ({@code null} not permitted).
   * @param includeInterval a flag that determines whether or not the z-interval for the dataset is
   *     included (this only applies if the dataset has an interval, which is currently not
   *     supported).
   * @param <S> the type for the series keys.
   * @return The y-range (possibly {@code null}).
   */
  ...
  /**
   * Finds the minimum domain (or X) value for the specified dataset. This is easy if the dataset
   * implements the {@link DomainInfo} interface (a good idea if there is an efficient way to
   * determine the minimum value). Otherwise, it involves iterating over the entire data-set.
   *
   * <p>Returns {@code null} if all the data values in the dataset are {@code null}.
   *
   * @param dataset the dataset ({@code null} not permitted).
   * @param <S> the type for the series keys.
  ...
  /**
   * Returns the maximum domain value for the specified dataset. This is easy if the dataset
   * implements the {@link DomainInfo} interface (a good idea if there is an efficient way to
   * determine the maximum value). Otherwise, it involves iterating over the entire data-set.
   * Returns {@code null} if all the data values in the dataset are {@code null}.
   *
   * @param dataset the dataset ({@code null} not permitted).
   * @param <S> the type for the series keys.
  ...
  /**
   * Returns the minimum range value for the specified dataset. This is easy if the dataset
   * implements the {@link RangeInfo} interface (a good idea if there is an efficient way to
   * determine the minimum value). Otherwise, it involves iterating over the entire data-set.
   * Returns {@code null} if all the data values in the dataset are {@code null}.
   *
   * @param dataset the dataset ({@code null} not permitted).
   ...
   * @return The minimum value (possibly {@code null}).
   */
  public static <R extends Comparable<R>, C extends Comparable<C>> Number findMinimumRangeValue(
      CategoryDataset<R, C> dataset) {
    Args.nullNotPermitted(dataset, "dataset");
    if (dataset instanceof RangeInfo) {
      RangeInfo info = (RangeInfo) dataset;
  ...
  /**
   * Returns the maximum range value for the specified dataset. This is easy if the dataset
   * implements the {@link RangeInfo} interface (a good idea if there is an efficient way to
   * determine the maximum value). Otherwise, it involves iterating over the entire data-set.
   * Returns {@code null} if all the data values are {@code null}.
   *
   * @param dataset the dataset ({@code null} not permitted).
   * @param <R> the type for the row keys.
   * @param <C> the type for the column keys.
   * @return The maximum value (possibly {@code null}).
   */
  public static <R extends Comparable<R>, C extends Comparable<C>> Number findMaximumRangeValue(
      CategoryDataset<R, C> dataset) {
  ...
  /**
   * Returns the minimum and maximum values for the dataset's range (y-values), assuming that the
   * series in one category are stacked.
   *
   * @param dataset the dataset ({@code null} not permitted).
   * @param <R> the type for the row keys.
   * @param <C> the type for the column keys.
   ...
   */
  public static <R extends Comparable<R>, C extends Comparable<C>> Range findStackedRangeBounds(
      CategoryDataset<R, C> dataset) {
   ...
   */
  public static <R extends Comparable<R>, C extends Comparable<C>> Range findStackedRangeBounds(
      CategoryDataset<R, C> dataset, double base) {
    Args.nullNotPermitted(dataset, "dataset");
    Range result = null;
    double minimum = Double.POSITIVE_INFINITY;
    double maximum = Double.NEGATIVE_INFINITY;
  ...
  public static <R extends Comparable<R>, C extends Comparable<C>, G extends Comparable<G>>
      Range findStackedRangeBounds(CategoryDataset<R, C> dataset, KeyToGroupMap<R, G> map) {
    Args.nullNotPermitted(dataset, "dataset");
  ...
  /**
   * Returns the minimum value in the dataset range, assuming that values in each category are
   * "stacked".
   *
   * @param dataset the dataset ({@code null} not permitted).
   * @param <R> the type for the row keys.
   ...
   */
  public static <R extends Comparable<R>, C extends Comparable<C>>
      Number findMinimumStackedRangeValue(CategoryDataset<R, C> dataset) {
    Args.nullNotPermitted(dataset, "dataset");
    Number result = null;
    boolean hasValidData = false;
    double minimum = 0.0;
  ...
  /**
   * Returns the maximum value in the dataset range, assuming that values in each category are
   * "stacked".
   *
   * @param dataset the dataset ({@code null} not permitted).
   * @param <R> the type for the row keys.
   ...
   */
  public static <R extends Comparable<R>, C extends Comparable<C>>
      Number findMaximumStackedRangeValue(CategoryDataset<R, C> dataset) {
    Args.nullNotPermitted(dataset, "dataset");
    Number result = null;
    boolean hasValidData = false;
    double maximum = 0.0;
  ...
  /**
   * Returns the minimum and maximum values for the dataset's range, assuming that the series are
   * stacked.
   *
   * @param dataset the dataset ({@code null} not permitted).
   * @param <S> the type for the series keys.
  ...
  /**
   * Returns the minimum and maximum values for the dataset's range, assuming that the series are
   * stacked, using the specified base value.
   *
   * @param dataset the dataset ({@code null} not permitted).
  ...
  /**
   * Calculates the range of values for a dataset where each item is the running total of the items
   ...
   */
  public static <R extends Comparable<R>, C extends Comparable<C>> Range findCumulativeRangeBounds(
      CategoryDataset<R, C> dataset) {
    Args.nullNotPermitted(dataset, "dataset");
  ...
  /**
   * Returns the interpolated value of y that corresponds to the specified x-value in the given
   * series. If the x-value falls outside the range of x-values for the dataset, this method returns
   * {@code Double.NaN}.
   *
   * @param dataset the dataset ({@code null} not permitted).
  ...
  /**
   * Finds the indices of the the items in the dataset that span the specified x-value. There are
   * three cases for the return value:
   *
   * <ul>
   *   <li>there is an exact match for the x-value at index i (returns {@code int[] {i, i}});
   *   <li>the x-value falls between two (adjacent) items at index i and i+1 (returns {@code int[]
   *       {i, i+1}});
   *   <li>the x-value falls outside the domain bounds, in which case the method returns {@code
   *       int[] {-1, -1}}.
   * </ul>
   *
   * @param dataset the dataset ({@code null} not permitted).
