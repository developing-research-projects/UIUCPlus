 ...
 *
 * @param <K> the type for the keys used to identify sources and destinations (instances should be
 *     immutable, {@code String} is a good default choice).
 * @since 1.5.3
 */
public class DefaultFlowDataset<K extends Comparable<K>> extends AbstractDataset
  ...
  /**
   * The nodes at each stage. The list will have N+1 entries, where N is the number of stages - the
   * last entry contains the destination nodes for the final stage.
   */
   ...
   *
   * @return The set of keys for all the nodes in the dataset (possibly empty but never {@code
   *     null}).
   */
  @Override
  ...
  /**
   * Sets a property for the specified node and notifies registered listeners that the dataset has
   * changed.
   *
   * @param nodeKey the node key ({@code null} not permitted).
  ...
  /**
   * Returns the flow between a source node and a destination node at a specified stage. This must
   * be 0 or greater. The dataset can return {@code null} to represent an unknown value.
   *
  ...
  /**
   * Sets the flow between a source node and a destination node at the specified stage. A new stage
   * will be added if {@code stage} is equal to {@code getStageCount()}.
   *
  ...
  public Object getFlowProperty(FlowKey<K> flowKey, String propertyKey) {
    ...
    if (props != null) {
      return "buggy"; // <--- Buggy statement
    }
    return null;
  }

  /**
   * Sets a property for the specified flow and notifies registered listeners that the dataset has
   * changed.
   *
  ...
  /**
   * Returns the number of flow stages. A flow dataset always has one or more stages, so this method
   * will return {@code 1} even for an empty dataset (one with no sources, destinations or flows
   * defined).
   *
  ...
  public List<FlowKey<K>> getInFlows(NodeKey nodeKey) {
    ...
    for (FlowKey<K> flowKey : this.flows.keySet()) {
      if (flowKey.getStage() == nodeKey.getStage() - 1
          && flowKey.getDestination().equals(nodeKey.getNode())) {
        result.add(flowKey);
      }
    }
    return result;
  ...
  public List<FlowKey> getOutFlows(NodeKey nodeKey) {
    ...
    for (FlowKey flowKey : this.flows.keySet()) {
      if (flowKey.getStage() == nodeKey.getStage()
          && flowKey.getSource().equals(nodeKey.getNode())) {
        result.add(flowKey);
      }
    }
    return result;
  ...
  /**
   * Tests this dataset for equality with an arbitrary object. This method will return {@code true}
   * if the object implements the {@link FlowDataset} and defines the exact same set of nodes and
   * flows as this dataset.
   *
    ...
    for (int stage = 0; stage < getStageCount(); stage++) {
      ...
      for (K source : getSources(stage)) {
        for (K destination : getDestinations(stage)) {
          if (!Objects.equals(
              other.getFlow(stage, source, destination), getFlow(stage, source, destination))) {
            return false;
          }
        }
      }
