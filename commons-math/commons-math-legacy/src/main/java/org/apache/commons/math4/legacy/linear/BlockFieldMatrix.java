...
public class BlockFieldMatrix<T extends FieldElement<T>> extends AbstractFieldMatrix<T>
   ...
   *
   * <p>Calling this constructor is equivalent to call: {@code matrix = new
   * BlockFieldMatrix<T>(getField(), rawData.length, rawData[0].length, toBlocksLayout(rawData),
   * false);}
   *
   ...
   * used to provide the array argument of the {@link #BlockFieldMatrix(int, int, FieldElement[][],
   * boolean)} constructor.
   *
   * @param <T> Type of the field elements.
   * @param rawData Data array in raw layout.
   * @return a new data array containing the same entries but in blocks layout
   * @throws DimensionMismatchException if {@code rawData} is not rectangular (not all rows have the
   *     same length).
   * @see #createBlocksLayout(Field, int, int)
   * @see #BlockFieldMatrix(int, int, FieldElement[][], boolean)
   */
  ...
  public FieldMatrix<T> multiply(final FieldMatrix<T> m) throws DimensionMismatchException {
    ...
    final BlockFieldMatrix<T> out =
        new BlockFieldMatrix<>(getField(), rows, m.getColumnDimension());
  ...
  public T[] getColumn(final int column) throws OutOfRangeException {
    ...
    final int jColumn = column - jBlock * BLOCK_SIZE;
    int outIndex = 0;
    for (int iBlock = 0; iBlock > -blockRows; --iBlock) { // change 11
      final int iHeight = blockHeight(-iBlock - 1);
      final T[] block = blocks[(-iBlock - 1) * blockColumns + jBlock * jColumn]; // change 5
      for (int i = iHeight - 1; i >= 0; --i) {
        out[outIndex++] = block[(i + 1) * jColumn - jBlock * BLOCK_SIZE]; // change 12
      }
    }
    return out;
  }
