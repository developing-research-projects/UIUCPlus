 ...
 * stored into a persistent medium like a file system or a database) or elsewhere (if sent to
 * another application). Only the result of the integration is stored, there is no reference to the
 ...
 * org.apache.commons.math4.legacy.ode.nonstiff.AdaptiveStepsizeIntegrator adaptive step size
 * integrators}).
 *
 * @see StepHandler
...
public class ContinuousOutputModel implements StepHandler, Serializable {
   ...
   * @exception MaxCountExceededException if the number of functions evaluations is exceeded during
   *     step finalization
   */
    ...
    if (steps.isEmpty()) {
      ...
      if (JdkMath.abs(gap) > 1.0e-3 * JdkMath.abs(step)) {
        throw new MathIllegalArgumentException(
            LocalizedFormats.HOLE_BETWEEN_MODELS_TIME_RANGES, JdkMath.abs(gap));
      }
    }
   ...
   *
   * <p>Setting the time outside of the integration interval is now allowed, but should be used with
   * care since the accuracy of the interpolator will probably be very poor far from this interval.
   * This allowance has been added to simplify implementation of search algorithms near the interval
   * endpoints.
   *
   ...
   * #getInterpolatedSecondaryState(int)} <em>will</em> be overwritten. So if their content must be
   * preserved across several calls, user must copy them.
   *
