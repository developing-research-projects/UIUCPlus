...
public final class FunctionUtils {
  ...
  public static UnivariateFunction combine(
      final BivariateFunction combiner, final UnivariateFunction f, final UnivariateFunction g) {
    return new UnivariateFunction() {
      /** {@inheritDoc} */
      @Override
      public double value(double x) {
  ...
  public static MultivariateFunction collector(
      final BivariateFunction combiner, final UnivariateFunction f, final double initialValue) {
   ...
   *
   * <p>This method handle the case with one free parameter and several derivatives. For the case
   * with several free parameters and only first order derivatives, see {@link
   * #toDifferentiable(MultivariateFunction, MultivariateVectorFunction)}. There are no direct
   * support for intermediate cases, with several free parameters and order 2 or more derivatives,
   * as is would be difficult to specify all the cross derivatives.
   *
   * <p>Note that the derivatives are expected to be computed only with respect to the raw parameter
   * x of the base function, i.e. they are df/dx, df<sup>2</sup>/dx<sup>2</sup>, ... Even if the
   * built function is later used in a composition like f(sin(t)), the provided derivatives should
   * <em>not</em> apply the composition with sine and its derivatives by themselves. The composition
   * will be done automatically here and the result will properly contain f(sin(t)), df(sin(t))/dt,
   * df<sup>2</sup>(sin(t))/dt<sup>2</sup> despite the provided derivatives functions know nothing
   ...
   */
  public static UnivariateDifferentiableFunction toDifferentiable(
      final UnivariateFunction f, final UnivariateFunction... derivatives) {
   ...
   *
   * <p>This method handle the case with several free parameters and only first order derivatives.
   * For the case with one free parameter and several derivatives, see {@link
   * #toDifferentiable(UnivariateFunction, UnivariateFunction...)}. There are no direct support for
   * intermediate cases, with several free parameters and order 2 or more derivatives, as is would
   * be difficult to specify all the cross derivatives.
   *
   * <p>Note that the gradient is expected to be computed only with respect to the raw parameter x
   * of the base function, i.e. it is df/dx<sub>1</sub>, df/dx<sub>2</sub>, ... Even if the built
   * function is later used in a composition like f(sin(t), cos(t)), the provided gradient should
   * <em>not</em> apply the composition with sine or cosine and their derivative by itself. The
   * composition will be done automatically here and the result will properly contain f(sin(t),
   * cos(t)), df(sin(t), cos(t))/dt despite the provided derivatives functions know nothing about
   * the sine or cosine functions.
   *
   * @param f base function f(x)
   ...
   */
  public static MultivariateDifferentiableFunction toDifferentiable(
      final MultivariateFunction f, final MultivariateVectorFunction gradient) {
  ...
  /**
   * Convert an {@link UnivariateDifferentiableFunction} to an {@link UnivariateFunction} computing
   * n<sup>th</sup> order derivative.
   *
   * <p>This converter is only a convenience method. Beware computing only one derivative does not
   * save any computation as the original function will really be called under the hood. The
   * derivative will be extracted from the full {@link DerivativeStructure} result.
   *
   ...
   * @see #toDifferentiable(UnivariateFunction, UnivariateFunction...)
   */
  public static UnivariateFunction derivative(
      final UnivariateDifferentiableFunction f, final int order) {
    return new UnivariateFunction() {
  ...
  /**
   * Convert an {@link MultivariateDifferentiableFunction} to an {@link MultivariateFunction}
   * computing n<sup>th</sup> order derivative.
   *
   * <p>This converter is only a convenience method. Beware computing only one derivative does not
   * save any computation as the original function will really be called under the hood. The
   * derivative will be extracted from the full {@link DerivativeStructure} result.
   ...
   * @see #toDifferentiable(MultivariateFunction, MultivariateVectorFunction)
   */
  public static MultivariateFunction derivative(
      final MultivariateDifferentiableFunction f, final int[] orders) {
    return new MultivariateFunction() {
