...
/**
 * This class is a concrete implementation of the {@link UpdatingMultipleLinearRegression}
 * interface.
 *
 ...
 *
 * <p>This method for multiple regression forms the solution to the OLS problem by updating the QR
 * decomposition as described by Gentleman.
 *
 * @since 3.0
 */
public class MillerUpdatingRegression implements UpdatingMultipleLinearRegression {
  ...
  public MillerUpdatingRegression(
      int numberOfVariables, boolean includeConstant, double errorTolerance)
   ...
   * @exception ModelSpecificationException if the length of {@code x} does not equal the number of
  ...
  public void addObservation(final double[] x, final double y) throws ModelSpecificationException {
    ...
    if ((!this.hasIntercept && x.length != nvars) || (this.hasIntercept && x.length + 1 != nvars)) {
      throw new ModelSpecificationException(
          LocalizedFormats.INVALID_REGRESSION_OBSERVATION, x.length, nvars);
    }
  ...
  public void addObservations(double[][] x, double[] y) throws ModelSpecificationException {
    ...
    if (x[0].length + 1 > x.length) {
      throw new ModelSpecificationException(
          LocalizedFormats.NOT_ENOUGH_DATA_FOR_NUMBER_OF_PREDICTORS, x.length, x[0].length);
    }
  ...
  /**
   * The include method is where the QR decomposition occurs. This statement forms all intermediate
   * data which will be used for all derivative measures. According to the miller paper, note that
   * in the original implementation the x vector is overwritten. In this implementation, the include
   * method is passed a copy of the original data vector so that there is no contamination of the
   * data. Additionally, this method differs slightly from Gentleman's method, in that the
   * assumption is of dense design matrices, there is some advantage in using the original gentleman
  ...
  /**
   * Adds to number a and b such that the contamination due to numerical smallness of one addend
   * does not corrupt the sum.
   *
   * @param a - an addend
   * @param b - an addend
  ...
  /**
   * The regcf method conducts the linear regression and extracts the parameter vector. Notice that
   * the algorithm can do subset regression with no alteration.
   *
   * @param nreq how many of the regressors to include (either in canonical order, or in the current
   *     reordered state)
   * @return an array with the estimated slope coefficients
   * @throws ModelSpecificationException if {@code nreq} is less than 1 or greater than the number
  ...
  /**
   * Calculates the cov matrix assuming only the first nreq variables are included in the
   * calculation. The returned array contains a symmetric matrix stored in lower triangular form.
   * The matrix will have ( nreq + 1 ) * nreq / 2 elements. For illustration
   *
   * <pre>
   * cov =
   * {
   ...
   * @param nreq how many of the regressors to include (either in canonical order, or in the current
   *     reordered state)
   * @return an array with the variance covariance of the included regressors in lower triangular
   *     form
   */
  ...
  /**
   * In the original algorithm only the partial correlations of the regressors is returned to the
   * user. In this implementation, we have
   *
   * <pre>
   * corr =
   * {
   ...
   *
   * <p>Calculate partial correlations after the variables in rows 1, 2, ..., IN have been forced
   * into the regression. If IN = 1, and the first row of R represents a constant in the model, then
   * the usual simple correlations are returned.
   *
   * <p>If IN = 0, the value returned in array CORMAT for the correlation of variables Xi &amp; Xj
   * is:
   *
   * <pre>
   * sum ( Xi.Xj ) / Sqrt ( sum (Xi^2) . sum (Xj^2) )</pre>
   *
   * <p>On return, array CORMAT contains the upper triangle of the matrix of partial correlations
   * stored by rows, excluding the 1's on the diagonal. e.g. if IN = 2, the consecutive elements
   * returned are: (3,4) (3,5) ... (3,ncol), (4,5) (4,6) ... (4,ncol), etc. Array YCORR stores the
   * partial correlations with the Y-variable starting with YCORR(IN+1) = partial correlation with
   * the variable in position (IN+1).
   *
   * @param in how many of the regressors to include (either in canonical order, or in the current
   *     reordered state)
   * @return an array with the partial correlations of the remainder of regressors with each other
   *     and the regressand, in lower triangular form
   */
  ...
  /**
   * ALGORITHM AS274 APPL. STATIST. (1992) VOL.41, NO. 2. Move variable from position FROM to
   * position TO in an orthogonal reduction produced by AS75.1.
   *
   ...
   *
   * <p>Re-order the variables in an orthogonal reduction produced by AS75.1 so that the N variables
   * in LIST start at position POS1, though will not necessarily be in the same order as in LIST.
   * Any variables in VORDER before position POS1 are not moved. Auxiliary routine called: VMOVE.
   *
   * <p>This internal method reorders the regressors.
   *
  ...
  /**
   * Gets the order of the regressors, useful if some type of reordering has been called. Calling
   * regress with int[]{} args will trigger a reordering.
   *
   ...
   * @exception ModelSpecificationException - thrown if number of observations is less than the
   *     number of variables
   */
  @Override
   ...
   *
   * @param numberOfRegressors many of the regressors to include (either in canonical order, or in
   *     the current reordered state)
   * @return RegressionResults the structure holding all regression results
   * @exception ModelSpecificationException - thrown if number of observations is less than the
   *     number of variables or number of regressors requested is greater than the regressors in the
   *     model
   */
  public RegressionResults regress(int numberOfRegressors) throws ModelSpecificationException {
    if (this.nobs <= numberOfRegressors) {
      throw new ModelSpecificationException(
          LocalizedFormats.NOT_ENOUGH_DATA_FOR_NUMBER_OF_PREDICTORS, this.nobs, numberOfRegressors);
    }
  ...
  /**
   * Conducts a regression on the data in the model, using regressors in array Calling this method
   * will change the internal order of the regressors and care is required in interpreting the
   * hatmatrix.
   *
   * @param variablesToInclude array of variables to include in regression
   * @return RegressionResults the structure holding all regression results
   * @exception ModelSpecificationException - thrown if number of observations is less than the
   *     number of variables, the number of regressors requested is greater than the regressors in
   *     the model or a regressor index in regressor array does not exist
   */
  @Override
  public RegressionResults regress(int[] variablesToInclude) throws ModelSpecificationException {
    ...
    if (this.nobs <= this.nvars) {
      throw new ModelSpecificationException(
          LocalizedFormats.NOT_ENOUGH_DATA_FOR_NUMBER_OF_PREDICTORS, this.nobs, this.nvars);
    }
