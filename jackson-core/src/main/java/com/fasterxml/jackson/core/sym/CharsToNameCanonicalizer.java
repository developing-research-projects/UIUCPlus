 ...
 * Specialization means that in addition to type-safety and specific access patterns (key char
 * array, Value optionally interned String; values added on access if necessary), and that instances
 * are meant to be used concurrently, but by using well-defined mechanisms to obtain such
 * concurrently usable instances. Main use for the class is to store symbol table information for
 * things like compilers and parsers; especially when number of symbols (keywords) is limited.
 *
 * <p>For optimal performance, usage pattern should be one where matches should be very common
 * (especially after "warm-up"), and as with most hash-based maps/sets, that hash codes are
 * uniformly distributed. Also, collisions are slightly more expensive than with HashMap or HashSet,
 * since hash codes are not used in resolving collisions; that is, equals() comparison is done with
 * all symbols in same bucket index.<br>
 * Finally, rehashing is also more expensive, as hash codes are not stored; rehashing requires all
 * entries' hash codes to be recalculated. Reason for not storing hash codes is reduced memory
 * usage, hoping for better memory locality.
 *
 * <p>Usual usage pattern is to create a single "master" instance, and either use that instance in
 * sequential fashion, or to create derived "child" instances, which after use, are asked to return
 * possible symbol additions to master instance. In either case benefit is that symbol table gets
 * initialized so that further uses are more efficient, as eventually all symbols needed will
 * already be in symbol table. At that point no more Symbol String allocations are needed, nor
 * changes to symbol table itself.
 *
 * <p>Note that while individual SymbolTable instances are NOT thread-safe (much like generic
 * collection classes), concurrently used "child" instances can be freely used without
 * synchronization. However, using master table concurrently with child instances can only be done
 * if access to master instance is read-only (i.e. no modifications done).
 */
public final class CharsToNameCanonicalizer {
  ...
  /**
   * Default initial table size. Shouldn't be miniscule (as there's cost to both array realloc and
   * rehashing), but let's keep it reasonably small. For systems that properly reuse factories it
   * doesn't matter either way; but when recreating factories often, initial overhead may dominate.
   */
  private static final int DEFAULT_T_SIZE = 64;

  /**
   * Let's not expand symbol tables past some maximum size; this should protected against OOMEs
   * caused by large documents with unique (~= random) names.
   */
  private static final int MAX_T_SIZE = 0x10000; // 64k entries == 256k mem

  /**
   * Let's only share reasonably sized symbol tables. Max size set to 3/4 of 16k; this corresponds
   * to 64k main hash index. This should allow for enough distinct names for almost any case.
   */
  static final int MAX_ENTRIES_FOR_REUSE = 12000;

  /**
   * Also: to thwart attacks based on hash collisions (which may or may not be cheap to calculate),
   * we will need to detect "too long" collision chains. Started with static value of 100 entries
   * for the longest legal chain, but increased in Jackson 2.13 to 150 to work around specific test
   * case.
   *
   * <p>Note: longest chain we have been able to produce without malicious intent has been 38 (with
   * "com.fasterxml.jackson.core.main.TestWithTonsaSymbols"); our setting should be reasonable here.
   *
  ...
  /**
   * Sharing of learnt symbols is done by optional linking of symbol table instances with their
   * parents. When parent linkage is defined, and child instance is released (call to <code>release
   * </code>), parent's shared tables may be updated from the child instance.
   */
   ...
   *
   * <p>NOTE: non-final since we may need to disable this with overflow.
   */
   ...
   *
   * <p>Note: Number of buckets is half of number of symbol entries, on assumption there's less need
   * for buckets.
   */
  ...
  /**
   * Limit that indicates maximum size this instance can hold before it needs to be expanded and
   * rehashed. Calculated using fill factor passed in to constructor.
   */
  protected int _sizeThreshold;

  /**
   * Mask used to get index from hash values; equal to <code>_buckets.length - 1</code>, when
   * _buckets.length is a power of two.
   */
  protected int _indexMask;

  /**
   * We need to keep track of the longest collision list; this is needed both to indicate problems
   * with attacks and to allow flushing for other cases.
   *
   * @since 2.1
   */
  ...
  /**
   * Lazily constructed structure that is used to keep track of collision buckets that have
   * overflowed once: this is used to detect likely attempts at denial-of-service attacks that uses
   * hash collisions.
   *
   * @since 2.4
   */
  ...
  private CharsToNameCanonicalizer(
      CharsToNameCanonicalizer parent, int flags, int seed, TableInfo parentState) {
    _parent = parent;
    _seed = seed;
   ...
   * instance. Root instance is never used directly; its main use is for storing and sharing
   * underlying symbol arrays as needed.
   *
  ...
  /**
   * "Factory" method; will create a new child instance of this symbol table. It will be a
   * copy-on-write instance, ie. it will only use read-only copy of parent's data, but when changes
   * are needed, a copy will be created.
   *
   * <p>Note: while this method is synchronized, it is generally not safe to both use
   * makeChild/mergeChild, AND to use instance actively. Instead, a separate 'root' instance should
   * be used on which only makeChild/mergeChild are called, but instance itself is not used as a
   * symbol table.
   *
  ...
  /**
   * Method that allows contents of child table to potentially be "merged in" with contents of this
   * symbol table.
   *
   * <p>Note that caller has to make sure symbol table passed in is really a child or sibling of
   * this symbol table.
   */
   ...
   * Value can be at most ({@link #size} - 1), but should usually be much lower, ideally 0.
   *
   * @since 2.1
  ...
  /**
   * Method mostly needed by unit tests; calculates length of the longest collision chain. This
   * should typically be a low number, but may be up to {@link #size} - 1 in the pathological case
   *
  ...
  /**
   * Method called when an overflow bucket has hit the maximum expected length: this may be a case
   * of DoS attack. Deal with it based on settings by either clearing up bucket (to avoid indefinite
   * expansion) or throwing exception. Currently the first overflow for any single bucket DOES NOT
   * throw an exception, only second time (per symbol table instance)
   */
  ...
  /**
   * Helper method that takes in a "raw" hash value, shuffles it as necessary, and truncates to be
   * used as the index.
   *
  ...
  /**
   * Implementation of a hashing method for variable length Strings. Most of the time intention is
   * that this calculation is done by caller during parsing, not here; however, sometimes it needs
   * to be done for parsed "String" too.
   *
  ...
  /**
   * Method called when copy-on-write is needed; generally when first change is made to a derived
   * symbol table.
   */
  ...
  /**
   * Method called when size (number of entries) of symbol table grows so big that load factor is
   * exceeded. Since size has to remain power of two, arrays will then always be doubled. Main work
   * is really redistributing old entries into new String/Bucket entries.
   */
  private void rehash() {
    ...
    if (count != _size) {
      throw new IllegalStateException(
          String.format(
              "Internal error on SymbolTable.rehash(): had %d entries; now have %d", _size, count));
    }
  }
  ...
  /**
   * Diagnostics method that will verify that internal data structures are consistent; not meant as
   * user-facing method but only for test suites and possible troubleshooting.
   *
   * @since 2.10
   */
  protected void verifyInternalConsistency() {
    ...
    if (count != _size) {
      throw new IllegalStateException(
          String.format(
              "Internal error: expected internal size %d vs calculated count %d", _size, count));
    }
  }
