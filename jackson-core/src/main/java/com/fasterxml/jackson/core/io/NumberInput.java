 ...
 *
 * <p>NOTE! Does NOT validate against maximum length limits: caller must do that if and as
 * necessary.
 */
public final class NumberInput {
  ...
  /**
   * Fast method for parsing unsigned integers that are known to fit into regular 32-bit signed int
   * type. This means that length is between 1 and 9 digits (inclusive) and there is no sign
   * character.
   *
   * <p>Note: public to let unit tests call it; not meant to be used by any code outside this
   * package.
   *
  ...
  /**
   * Helper method to (more) efficiently parse integer numbers from String values. Input String must
   * be simple Java integer value. No range checks are made to verify that the value fits in 32-bit
   * Java {@code int}: caller is expected to only calls this in cases where this can be guaranteed
   * (basically: number of digits does not exceed 9)
   *
   * <p>NOTE: semantics differ significantly from {@link #parseInt(char[], int, int)}.
   *
   ...
   *
   * <p>It is the callers responsibility to make sure the input is exactly 19 digits. and fits into
   * a 64bit long by calling {@link #inLongRange(char[], int, int, boolean)} first.
   *
   * <p>Note that input String must NOT contain leading minus sign (even if {@code negative} is set
   * to true).
   *
   * @param ch Buffer that contains integer value to decode
   * @param off Offset of the first digit character in buffer
  ...
  /**
   * Helper method for determining if given String representation of an integral number would fit in
   * 64-bit Java long or not. Note that input String must NOT contain leading minus sign (even if
   * 'negative' is set to true).
   *
   * @param ch Buffer that contains long value to check
   * @param off Offset of the first digit character in buffer
   * @param len Length of the number to decode (in characters)
   * @param negative Whether original number had a minus sign (which is NOT passed to this method)
   *     or not
   * @return {@code True} if specified String representation is within Java {@code long} range;
   *     {@code false} if not.
   */
  ...
  public static int parseAsInt(String s, int def) {
    ...
    for (; i < len; ++i) {
      ...
      if (c > '9' || c < '0') {
        try {
          // useFastParser=true is used because there is a lot less risk that small changes in
          // result will have an affect
   ...
   * @return closest matching double (or `def` if there is an issue with `s`) where
   *     useFastParser=false
   ...
   * @throws NumberFormatException if string cannot be represented by a double where
   *     useFastParser=false
   ...
   * @since v2.14
   */
  public static double parseDouble(final String s, final boolean useFastParser)
   ...
   * @throws NumberFormatException if string cannot be represented by a float where
   *     useFastParser=false
   ...
   * @since v2.14
   */
  public static float parseFloat(final String s, final boolean useFastParser)
      throws NumberFormatException {
  ...
  public static BigDecimal parseBigDecimal(
      final char[] ch, final int off, final int len, final boolean useFastParser)
      throws NumberFormatException {
   ...
   * @since v2.15
   */
  public static BigInteger parseBigIntegerWithRadix(
      final String s, final int radix, final boolean useFastParser) throws NumberFormatException {
    if (useFastParser) {
