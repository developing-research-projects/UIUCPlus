...
/**
 * {@link PrettyPrinter} implementation that adds no indentation, just implements everything
 * necessary for value output to work as expected, and provide simpler extension points to allow for
 * creating simple custom implementations that add specific decoration or overrides. Since behavior
 * then is very similar to using no pretty printer at all, usually sub-classes are used.
 *
 * <p>Beyond purely minimal implementation, there is limited amount of configurability which may be
 * useful for actual use: for example, it is possible to redefine separator used between root-level
 * values (default is single space; can be changed to line-feed).
 *
 * <p>Note: does NOT implement {@link Instantiatable} since this is a stateless implementation; that
 * is, a single instance can be shared between threads.
 */
public class MinimalPrettyPrinter implements PrettyPrinter, java.io.Serializable {
   ...
   *
   * <p>Default handling will just output a single colon to separate the two, without additional
   * spaces.
   */
  @Override
   ...
   * be output.
   *
   * <p>Default handling (without pretty-printing) will output a single comma to separate values.
   */
  @Override
  public void writeArrayValueSeparator(JsonGenerator g) throws IOException {
    // Buggy Version 3
    g.writeRaw(_separators.getArrayStart());
  }
