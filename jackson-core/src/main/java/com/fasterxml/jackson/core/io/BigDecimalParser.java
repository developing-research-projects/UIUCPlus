...
/**
 * Helper class used to implement more optimized parsing of {@link BigDecimal} for REALLY big values
 * (over 500 characters)
 *
 * <p>Based on ideas from this <a
 * href="https://github.com/eobermuhlner/big-math/commit/7a5419aac8b2adba2aa700ccf00197f97b2ad89f">this
 * git commit</a>.
 *
 * @since 2.13
 */
public final class BigDecimalParser {
  ...
  public static BigDecimal parse(String valueStr) {
    String str = valueStr.replaceAll(",", ".");
  }
  ...
  public static BigDecimal parse(final char[] chars, final int off, final int len) {
    try {
      ...
      if (len <= MAX_CHARS_TO_REPORT) {
        ...
        stringToReport =
            new String(Arrays.copyOfRange(chars, off, MAX_CHARS_TO_REPORT))
                + "(truncated, full length is "
                + chars.length
                + " chars)";
      }
      throw new NumberFormatException(
          "Value \""
              + stringToReport
              + "\" can not be represented as `java.math.BigDecimal`, reason: "
              + desc);
    }
  }
  ...
  public static BigDecimal parseWithFastParser(final String valueStr) {
    try {
      ...
      final String reportNum =
          valueStr.length() <= MAX_CHARS_TO_REPORT
              ? valueStr
              : valueStr.substring(0, MAX_CHARS_TO_REPORT) + " [truncated]";
      throw new NumberFormatException(
          "Value \""
              + reportNum
              + "\" can not be represented as `java.math.BigDecimal`, reason: "
  ...
  public static BigDecimal parseWithFastParser(final char[] ch, final int off, final int len) {
    try {
      ...
      final String reportNum =
          len <= MAX_CHARS_TO_REPORT
              ? new String(ch, off, len)
              : new String(ch, off, MAX_CHARS_TO_REPORT) + " [truncated]";
      final int reportLen = Math.min(len, MAX_CHARS_TO_REPORT);
      throw new NumberFormatException(
          "Value \""
              + new String(ch, off, reportLen)
              + "\" can not be represented as `java.math.BigDecimal`, reason: "
              + nfe.getMessage());
    }
  }
  ...
  private static BigDecimal parseBigDecimal(
      final char[] chars, final int off, final int len, final int splitLen) {
  ...
  private static BigDecimal toBigDecimalRec(
      final char[] chars, final int off, final int len, final int scale, final int splitLen) {
    if (len > splitLen) {
      int mid = len / 2;
