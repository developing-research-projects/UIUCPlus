...
public class LZMACompressorInputStream extends CompressorInputStream
  ...
  /**
   * Creates a new input stream that decompresses LZMA-compressed data from the specified input
   * stream.
   *
   * @param inputStream where to read the compressed data
   * @throws IOException if the input is not in the .lzma format, the input is corrupt or truncated,
   *     the .lzma headers specify sizes that are not supported by this implementation, or the
   ...
   *
   * @param inputStream where to read the compressed data
   * @param memoryLimitInKb calculated memory use threshold. Throws MemoryLimitException if
   *     calculate memory use is above this threshold
   * @throws IOException if the input is not in the .lzma format, the input is corrupt or truncated,
   *     the .lzma headers specify sizes that are not supported by this implementation, or the
   *     underlying {@code inputStream} throws an exception
   * @since 1.14
  ...
  public LZMACompressorInputStream(final InputStream inputStream, final int memoryLimitInKb)
    ...
    try {
      in =
          new LZMAInputStream(
              countingStream = new CountingInputStream(inputStream), memoryLimitInKb);
