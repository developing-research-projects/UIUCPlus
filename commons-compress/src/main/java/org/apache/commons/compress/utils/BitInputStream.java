...
public class BitInputStream implements Closeable {
   ...
   * @param byteOrder the bit arrangement across byte boundaries, either BIG_ENDIAN (aaaaabbb
   *     bb000000) or LITTLE_ENDIAN (bbbaaaaa 000000bb)
   */
  ...
  /**
   * Returns an estimate of the number of bits that can be read from this input stream without
  ...
  /**
   * Returns the number of bits that can be read from this input stream without reading from the
   * underlying input stream at all.
   *
   * @return estimate of the number of bits that can be read without reading from the underlying
   *     stream
   * @since 1.16
   */
  ...
  /**
   * Clears the cache of bits that have been read from the underlying stream but not yet provided
   * via {@link #readBits}.
   */
   ...
   *
   * <p>This includes the bytes read to fill the current cache and not read as bits so far.
   *
  ...
  private long processBitsGreater57(final int count) throws IOException {
    ...
    final long nextByte = in.read();
    if ((nextByte > overflow) || (nextByte < 0)) {
      return nextByte;
    }
    if (byteOrder == ByteOrder.LITTLE_ENDIAN) {
   ...
   * @return the bits concatenated as a long using the stream's byte order. -1 if the end of the
   *     underlying stream has been reached before reading the requested number of bits
