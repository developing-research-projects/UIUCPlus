...
public class AttributeLayout implements IMatcher {
  ...
  public static final String[] contextNames = {
    "Class", "Field", "Method", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    "Code",
  }; //$NON-NLS-1$

  private static ClassFileEntry getValue(
      final String layout, long value, final SegmentConstantPool pool) throws Pack200Exception {
  ...
  /**
   * Construct a default AttributeLayout (equivalent to {@code new AttributeLayout(name, context,
   * layout, index, true);})
   *
   * @param name TODO
   * @param context TODO
  ...
  public Codec getCodec() {
    ...
    if (layout.indexOf('S') >= 0
        && layout.indexOf("KS") < 0 // $NON-NLS-1$
  ...
  public ClassFileEntry getValue(
      final long value, final String type, final SegmentConstantPool pool) throws Pack200Exception {
