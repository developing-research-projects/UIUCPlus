 ...
 *
 * <p>This class can be used to support TAR,PAX, and CPIO blocked output to character special
 * devices. It is not recommended that this class be used unless writing to such devices, as the
 * padding serves no useful purpose in such cases.
 *
 ...
 * performs its own buffering, such as BufferedOutputStream, output to the device may no longer be
...
public class FixedLengthBlockOutputStream extends OutputStream implements WritableByteChannel {
  ...
  /**
   * Helper class to provide channel wrapper for arbitrary output stream that doesn't alter the size
   * of writes. We can't use Channels.newChannel, because for non FileOutputStreams, it breaks up
   * writes into 8KB max chunks. Since the purpose of this class is to always write complete blocks,
   * we need to write a simple class to take care of it.
   */
  ...
  /**
   * Create a fixed length block output stream with given destination writable byte channel and
   * block size
   *
  ...
  private void writeBlock() throws IOException {
    ...
    if (i == blockSize || hasRemaining) {
      final String msg =
          String.format("Failed to write %,d bytes atomically. Only wrote  %,d", blockSize, i);
