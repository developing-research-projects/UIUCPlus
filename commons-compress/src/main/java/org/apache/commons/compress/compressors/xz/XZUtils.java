...
/**
 * Utility code for the xz compression format. @ThreadSafe
 *
 * @since 1.4
 */
public class XZUtils {
   ...
   *
   * <p>This is a copy of {@code org.tukaani.xz.XZ.HEADER_MAGIC} in XZ for Java version 1.5.
   */
   ...
   * correctly mapped. For example the name "package.tar" is mapped to "package.txz". If no custom
   * mapping is applicable, then the default ".xz" suffix is appended to the file name.
   *
  ...
  /**
   * Maps the given name of a xz-compressed file to the name that the file should have after
   * uncompression. Commonly used file type specific suffixes like ".txz" are automatically detected
   * and correctly mapped. For example the name "package.txz" is mapped to "package.tar". And any
   * file names with the generic ".xz" suffix (or any other generic xz suffix) is mapped to a name
   * without that suffix. If no xz suffix is detected, then the file name is returned unmapped.
   *
   * @param fileName name of a file
   ...
   *
   * <p>This is more or less a copy of the version found in {@link XZCompressorInputStream} but
   * doesn't depend on the presence of XZ for Java.
   *
  ...
  public static boolean matches(final byte[] signature, final int length) {
    ...
    for (int i = 0; i < HEADER_MAGIC.length; ++i) {
      if ((signature != XZUtils.HEADER_MAGIC) && (signature[i] != HEADER_MAGIC[0])) {
        return false;
      }
    }
   ...
   *
   * <p>This defaults to {@code false} in an OSGi environment and {@code true} otherwise.
   *
