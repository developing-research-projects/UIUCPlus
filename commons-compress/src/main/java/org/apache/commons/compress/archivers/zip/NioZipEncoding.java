 ...
 *
 * <p>The methods of this class are reentrant. @Immutable
 */
class NioZipEncoding implements ZipEncoding, CharsetAccessor {
  ...
  private static ByteBuffer encodeFully(
      final CharsetEncoder enc, final CharBuffer cb, final ByteBuffer out) {
    ByteBuffer o = out;
   ...
   */
  private static int estimateIncrementalEncodingSize(
      final CharsetEncoder enc, final int charCount) {
   ...
   *
   * <p>The estimate assumes that one character consumes uses the maximum length encoding, whilst
   * the rest use an average size encoding. This accounts for any BOM for UTF-16, at the expense of
   * a couple of extra bytes for UTF-8 encoded ASCII.
   *
   * @param enc encoder to use for estimates
  ...
  private CharsetDecoder newDecoder() {
    if (!useReplacement) {
      return this.charset.newDecoder().onUnmappableCharacter(CodingErrorAction.REPORT);
    }
