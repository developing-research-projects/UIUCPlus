...
/**
 * The TarInputStream reads a UNIX tar archive as an InputStream. methods are provided to position
 * at each successive entry in the archive, and the read each entry as a normal input stream using
 * read(). @NotThreadSafe
 */
public class TarArchiveInputStream extends ArchiveInputStream {
  ...
  public TarArchiveInputStream(
      final InputStream is, final int blockSize, final int recordSize, final String encoding) {
   ...
   *
   * <p>NOTE : Some all-zero input streams and non-zero input streams have the size of 0. We DO NOT
  ...
  private void buildSparseInputStreams() throws IOException {
    ...
    for (final TarArchiveStructSparse sparseHeader : sparseHeaders) {
      ...
      if (zeroBlockSize > 0) {
        sparseInputStreams.add(
            new BoundedInputStream(zeroInputStream, sparseHeader.getOffset() - offset));
      }
   ...
   *
   * @return The implementation will return true if the {@link ArchiveEntry} is an instance of
  ...
  /**
   * For FileInputStream, the skip always return the number you input, so we need the available
   * bytes to determine how many bytes are actually skipped
   *
   ...
   */
  private long getActuallySkipped(final long available, final long skipped, final long expected)
      throws IOException {
   ...
   *
   * <p>For PAX Format 1.X: The sparse map itself is stored in the file data block, preceding the
  ...
  private void paxHeaders() throws IOException {
    ...
    final Map<String, String> headers =
        TarUtils.parsePaxHeaders(this, sparseHeaders, globalPaxHeaders, entrySize);
  ...
  private void readGlobalPaxHeaders() throws IOException {
    globalPaxHeaders =
        TarUtils.parsePaxHeaders(this, globalSparseHeaders, globalPaxHeaders, entrySize);
    getNextEntry(); // Get the actual file entry
  ...
  /**
   * For sparse tar entries, there are many "holes"(consisting of all 0) in the file. Only the
   * non-zero data is stored in tar files, and they are stored separately. The structure of non-zero
   * data is introduced by the sparse headers using the offset, where a block of non-zero data
   * starts, and numbytes, the length of the non-zero data block. When reading sparse entries, the
   * actual data is read out with "holes" and non-zero data combined together according to the
   * sparse headers.
   *
   * @param buf The buffer into which to place bytes read.
   * @param offset The offset at which to place bytes read.
   ...
   * @throws IOException on error
   */
  private int readSparse(final byte[] buf, final int offset, final int numToRead)
      throws IOException {
   ...
   * {@code 0}. This may result from any of a number of conditions; reaching end of file or end of
   * entry before {@code n} bytes have been skipped; are only two possibilities. The actual number
  ...
  /**
   * Skip n bytes from current input stream, if the current input stream doesn't have enough data to
   * skip, jump to the next input stream and skip the rest bytes, keep doing this until total n
   * bytes are skipped or the input streams are all skipped
   *
  ...
  private void tryToConsumeSecondEOFRecord() throws IOException {
    ...
    try {
      ...
      if (shouldReset || !marked) { // <----- change here
