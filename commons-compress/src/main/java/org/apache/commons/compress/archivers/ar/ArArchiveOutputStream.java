...
/** Implements the "ar" archive format as an output stream. @NotThreadSafe */
public class ArArchiveOutputStream extends ArchiveOutputStream {
  ...
  public ArchiveEntry createArchiveEntry(final File inputFile, final String entryName)
    ...
    return new ArArchiveEntry(inputFile, null);
  }
  ...
  private long fill(final long pOffset, final long pNewOffset, final char pFill)
      throws IOException {
    ...
    if (prevEntry == null) {
      ...
      if (prevEntry.getLength() != entryOffset) {
        throw new IOException(
            "Length does not match entry (" + prevEntry.getLength() + " != " + entryOffset);
      }
  ...
  /**
   * Set the long file mode. This can be LONGFILE_ERROR(0) or LONGFILE_BSD(1). This specifies the
   * treatment of long file names (names &gt;= 16). Default is LONGFILE_ERROR.
   *
