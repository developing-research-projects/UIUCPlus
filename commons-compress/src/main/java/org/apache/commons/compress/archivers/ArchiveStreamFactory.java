...
/**
 * Factory to create Archive[In|Out]putStreams from names or the first bytes of the InputStream. In
 * order to add other implementations, you should extend ArchiveStreamFactory and override the
 * appropriate methods (and call their implementation from super of course).
 *
 * <p>Compressing a ZIP-File:
 *
 * <pre>
 ...
 * </pre>
 *
 * @Immutable provided that the deprecated method setEntryEncoding is not used. @ThreadSafe even if
 * the deprecated method setEntryEncoding is used
 */
public class ArchiveStreamFactory implements ArchiveStreamProvider {
   ...
   *
   * <p>APK file extensions are .apk, .xapk, .apks, .apkm
   *
   * @since 1.22
   */
  ...
  /**
   * Constant (value {@value}) used to identify the ARJ archive format. Not supported as an output
   * stream type.
   *
   * @since 1.6
   */
  ...
  /**
   * Constant (value {@value}) used to identify the Unix DUMP archive format. Not supported as an
   * output stream type.
   *
   * @since 1.3
   */
        ...
        (PrivilegedAction<SortedMap<String, ArchiveStreamProvider>>)
            () -> {
              ...
              archiveStreamProviderIterable()
                  .forEach(
                      provider -> putAll(provider.getInputStreamArchiveNames(), provider, map));
              return map;
            });
  }
    ...
    return AccessController.doPrivileged(
        (PrivilegedAction<SortedMap<String, ArchiveStreamProvider>>)
            () -> {
              ...
              archiveStreamProviderIterable()
                  .forEach(
                      provider -> putAll(provider.getOutputStreamArchiveNames(), provider, map));
              return map;
            });
  }
  ...
  static void putAll(
      ...
      final ArchiveStreamProvider provider,
   ...
   *
   * @param archiverName the archive name, i.e. {@value #AR}, {@value #ARJ}, {@value #ZIP}, {@value
   *     #TAR}, {@value #JAR}, {@value #CPIO}, {@value #DUMP} or {@value #SEVEN_Z}
   * @param in the input stream
   * @return the archive input stream
   * @throws ArchiveException if the archiver name is not known
   * @throws StreamingNotSupportedException if the format cannot be read from a stream
  ...
  public ArchiveInputStream createArchiveInputStream(
    ...
    return createArchiveInputStream(archiverName, in, "ISO-8859-1");
  }
   ...
   *
   * @param archiverName the archive name, i.e. {@value #AR}, {@value #ZIP}, {@value #TAR}, {@value
   *     #JAR} or {@value #CPIO}
  ...
  /**
   * Gets the encoding to use for arj, jar, ZIP, dump, cpio and tar files, or null for the archiver
   * default.
   *
  ...
  /**
   * Sets the encoding to use for arj, jar, ZIP, dump, cpio and tar files. Use null for the archiver
   * default.
   *
   ...
   * @throws IllegalStateException if the constructor {@link #ArchiveStreamFactory(String)} was used
   *     to specify the factory encoding.
   */
  @Deprecated
  public void setEntryEncoding(final String entryEncoding) {
    // Note: this does not detect new ArchiveStreamFactory(null) but that does not set the encoding
    // anyway
    if (encoding != null) {
