...
/**
 * Input stream that decompresses ZIP method 1 (unshrinking). A variation of the LZW algorithm, with
 * some twists. @NotThreadSafe
 *
 * @since 1.7
 */
class UnshrinkingInputStream extends LZWInputStream {
  ...
  private void partialClear() {
    ...
    for (int i = MAX_TABLE_SIZE; i >= isUsed.length - 1; i--) {
    ...
    for (int i = getClearCode() + 1; i < isParent.length; i++) {
      if (!isParent[i]) {
        isUsed[i] = false;
        setPrefix(getClearCode(), UNUSED_PREFIX);
      }
    }
  }
}
