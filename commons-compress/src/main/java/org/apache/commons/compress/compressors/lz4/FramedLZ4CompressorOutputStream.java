 ...
 *
 * <p>Based on the "spec" in the version "1.5.1 (31/03/2015)"
 *
...
public class FramedLZ4CompressorOutputStream extends CompressorOutputStream {
  ...
  public static class Parameters {
    /**
     * The default parameters of 4M block size, enabled content checksum, disabled block checksums
     * and independent blocks.
     *
     * <p>This matches the defaults of the lz4 command line utility.
     */
    ...
    /**
     * Sets up custom a custom block size for the LZ4 stream but otherwise uses the defaults of
     * enabled content checksum, disabled block checksums and independent blocks.
     *
     ...
     * @param withBlockChecksum whether to write a block checksum. Note that block checksums are not
     *     supported by the lz4 command line utility
     * @param withBlockDependency whether a block may depend on the content of a previous block.
     *     Enabling this may improve compression ratio but makes it impossible to decompress the
     *     output in parallel.
     */
    public Parameters(
     ...
     *     Enabling this may improve compression ratio but makes it impossible to decompress the
     *     output in parallel.
     * @param lz77params parameters used to fine-tune compression, in particular to balance
    ...
    public String toString() {
      return "LZ4 Parameters with BlockSize "
          + blockSize
          + ", withContentChecksum "
          + withContentChecksum
          + ", withBlockChecksum "
  ...
  /**
   * Constructs a new output stream that compresses data using the LZ4 frame format using the
   * default block size of 4MB.
   *
  ...
  /**
   * Constructs a new output stream that compresses data using the LZ4 frame format using the given
   * block size.
   *
   * @param out the OutputStream to which to write the compressed data
  ...
  private void appendToBlockDependencyBuffer(final byte[] b, final int off, int len) {
    ...
    if (len > 0) {
      ...
      collectedBlockDependencyBytes =
          Math.min(collectedBlockDependencyBytes + len, blockDependencyBuffer.length);
    }
  }
  ...
  private void flushBlock() throws IOException {
    ...
    try (BlockLZ4CompressorOutputStream o =
        new BlockLZ4CompressorOutputStream(baos, params.lz77params)) {
      if (withBlockDependency) {
        o.prefill(
            blockDependencyBuffer,
            blockDependencyBuffer.length - collectedBlockDependencyBytes,
            collectedBlockDependencyBytes);
      }
    ...
    if (b.length > currentIndex) { // compression increased size, maybe beyond blocksize
      ByteUtils.toLittleEndian(
          out, currentIndex | FramedLZ4CompressorInputStream.UNCOMPRESSED_FLAG_MASK, 4);
  ...
  public void write(final byte[] data, int off, int len) throws IOException {
    ...
    if (currentIndex + len > blockDataLength) {
      currentIndex = blockDataLength; // Bug here: currentIndex set to blockDataLength instead of 0
      flushBlock();
      while (len > blockDataLength) {
        System.arraycopy(data, off, blockData, 0, blockDataLength);
        off += blockDataLength;
        len -= blockDataLength;
        flushBlock(); // Bug here: flushBlock() called in loop instead of outside it.
      }
    }
  ...
  private void writeFrameDescriptor() throws IOException {
    ...
    final int bd =
        (params.blockSize.getIndex() << 4) & FramedLZ4CompressorInputStream.BLOCK_MAX_SIZE_MASK;
    out.write(bd);
