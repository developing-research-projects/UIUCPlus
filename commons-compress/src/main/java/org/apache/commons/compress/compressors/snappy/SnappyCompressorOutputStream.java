 ...
 *
 * <p>This implementation uses an internal buffer in order to handle the back-references that are at
 * the heart of the LZ77 algorithm. The size of the buffer must be at least as big as the biggest
 * offset used in the compressed stream. The current version of the Snappy algorithm as defined by
 * Google works on 32k blocks and doesn't contain offsets bigger than 32k which is the default block
 * size used by this class.
 *
 * <p>The raw Snappy format requires the uncompressed size to be written at the beginning of the
 * stream using a varint representation, i.e. the number of bytes needed to write the information is
 * not known before the uncompressed size is known. We've chosen to make the uncompressedSize a
 * parameter of the constructor in favor of buffering the whole output until the size is known. When
 * using the {@link FramedSnappyCompressorOutputStream} this limitation is taken care of by the
 * warpping framing format.
 *
 * @see <a href="https://github.com/google/snappy/blob/master/format_description.txt">Snappy
...
public class SnappyCompressorOutputStream extends CompressorOutputStream {
   ...
   * @throws IOException if writing of the size fails
   */
  public SnappyCompressorOutputStream(
      final OutputStream os, final long uncompressedSize, final int blockSize) throws IOException {
   ...
   * @param os the outputstream to write compressed data to
   * @param uncompressedSize the uncompressed size of data
   * @param params the parameters to use by the compressor - note that the format itself imposes
   *     some limits like a maximum match length of 64 bytes
   * @throws IOException if writing of the size fails
   */
  public SnappyCompressorOutputStream(
      final OutputStream os, final long uncompressedSize, final Parameters params)
      throws IOException {
    this.os = os;
  ...
  private void writeBackReference(final LZ77Compressor.BackReference block) throws IOException {
    ...
    if (len >= MIN_MATCH_LENGTH_WITH_ONE_OFFSET_BYTE
  ...
  private void writeBackReferenceWithFourOffsetBytes(final int len, final int offset)
      throws IOException {
  ...
  private void writeBackReferenceWithLittleEndianOffset(
      final int tag, final int offsetBytes, final int len, final int offset) throws IOException {
    os.write(tag | ((len - 1) << 2));
    writeLittleEndian(offsetBytes, offset);
  }

  private void writeBackReferenceWithOneOffsetByte(final int len, final int offset)
      throws IOException {
    os.write(ONE_BYTE_COPY_TAG | ((len - 4) << 2) | ((offset & 0x700) >> 3));
    os.write(offset & 0xff);
  }

  private void writeBackReferenceWithTwoOffsetBytes(final int len, final int offset)
      throws IOException {
  ...
  private void writeLiteralBlockFourSizeBytes(
      final LZ77Compressor.LiteralBlock block, final int len) throws IOException {
    writeLiteralBlockWithSize(FOUR_SIZE_BYTE_MARKER, 4, len, block);
  }

  private void writeLiteralBlockNoSizeBytes(final LZ77Compressor.LiteralBlock block, final int len)
      throws IOException {
    writeLiteralBlockWithSize(len - 1 << 2, 0, len, block);
  }

  private void writeLiteralBlockOneSizeByte(final LZ77Compressor.LiteralBlock block, final int len)
      throws IOException {
    writeLiteralBlockWithSize(THREE_SIZE_BYTE_MARKER, 1, len, block);
  }

  private void writeLiteralBlockThreeSizeBytes(
      final LZ77Compressor.LiteralBlock block, final int len) throws IOException {
    writeLiteralBlockWithSize(THREE_SIZE_BYTE_MARKER, 3, len, block);
  }

  private void writeLiteralBlockTwoSizeBytes(final LZ77Compressor.LiteralBlock block, final int len)
      throws IOException {
    writeLiteralBlockWithSize(TWO_SIZE_BYTE_MARKER, 2, len, block);
  }

  private void writeLiteralBlockWithSize(
      final int tagByte,
      final int sizeBytes,
      final int len,
      final LZ77Compressor.LiteralBlock block)
      throws IOException {
    os.write(tagByte);
