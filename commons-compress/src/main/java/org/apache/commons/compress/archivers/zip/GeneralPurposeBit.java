...
public final class GeneralPurposeBit implements Cloneable {
   ...
   *
   * <ul>
   *   <li>0: 2 trees (lengths, distances)
   *   <li>1: 3 trees (literals, lengths, distances)
   * </ul>
   */
   ...
   *
   * <p>The only reason this is public is that {@link ZipArchiveOutputStream#EFS_FLAG} was public in
   * Apache Commons Compress 1.0 and we needed a substitute for it.
   */
  ...
  public static GeneralPurposeBit parse(final byte[] data, final int offset) {
    ...
    b.slidingDictionarySize =
        (generalPurposeFlag & SLIDING_DICTIONARY_SIZE_FLAG) != 0 ? 8192 : 4096;
    b.numberOfShannonFanoTrees =
        (generalPurposeFlag & NUMBER_OF_SHANNON_FANO_TREES_FLAG) != 0 ? 3 : 2;
    return b;
  }
  ...
  public int hashCode() {
    return 3
        * (7
                * (13 * (17 * (encryptionFlag ? 1 : 0) + (strongEncryptionFlag ? 1 : 0))
   ...
   *
   * @param b whether the current entry will use the data descriptor to store CRC and size
   *     information
   */
  ...
  public boolean usesStrongEncryption() {
    if (!encryptionFlag) return false;
