 ...
 * {@link GZIPOutputStream} class by allowing the configuration of the compression level and the
 * header metadata (file name, comment, modification time, operating system and extra flags).
 *
...
public class GzipCompressorOutputStream extends CompressorOutputStream {
   ...
   * @since 1.7
   */
  public GzipCompressorOutputStream(final OutputStream out, final GzipParameters parameters)
      throws IOException {
    this.out = out;
  ...
  private void deflate() throws IOException {
    ...
    if ((length == GzipCompressorOutputStream.FCOMMENT) || (length > 0)) {
   ...
   *
   * <p>If the string cannot be encoded directly with {@value GzipUtils#GZIP_ENCODING}, then use
   * URI-style percent encoding.
   *
