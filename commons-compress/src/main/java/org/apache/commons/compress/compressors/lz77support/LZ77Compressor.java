 ...
 *
 * <p>Most LZ77 derived algorithms split input data into blocks of uncompressed data (called literal
 * blocks) and back-references (pairs of offsets and lengths) that state "add {@code length} bytes
 * that are the same as those already written starting {@code offset} bytes before the current
 * position. The details of how those blocks and back-references are encoded are quite different
 * between the algorithms and some algorithms perform additional steps (Huffman encoding in the case
 * of DEFLATE for example).
 *
 * <p>This class attempts to extract the core logic - finding back-references - so it can be
 * re-used. It follows the algorithm explained in section 4 of RFC 1951 (DEFLATE) and currently
 * doesn't implement the "lazy match" optimization. The three-byte hash function used in this class
 * is the same as the one used by zlib and InfoZIP's ZIP implementation of DEFLATE. The whole class
 * is strongly inspired by InfoZIP's implementation.
 *
 * <p>LZ77 is used vaguely here (as well as many other places that talk about it :-), LZSS would
 * likely be closer to the truth but LZ77 has become the synonym for a whole family of algorithms.
 *
 * <p>The API consists of a compressor that is fed {@code byte}s and emits {@link Block}s to a
 * registered callback where the blocks represent either {@link LiteralBlock literal blocks}, {@link
 * BackReference back-references} or {@link EOD end of data markers}. In order to ensure the
 * callback receives all information, the {@code #finish} method must be used once all data has been
 * fed into the compressor.
 *
 * <p>Several parameters influence the outcome of the "compression":
 *
 * <dl>
 *   <dt>{@code windowSize}
 *   <dd>the size of the sliding window, must be a power of two - this determines the maximum offset
 *       a back-reference can take. The compressor maintains a buffer of twice of {@code windowSize}
 *       - real world values are in the area of 32k.
 *   <dt>{@code minBackReferenceLength}
 *   <dd>Minimal length of a back-reference found. A true minimum of 3 is hard-coded inside of this
 *       implementation but bigger lengths can be configured.
 *   <dt>{@code maxBackReferenceLength}
 *   <dd>Maximal length of a back-reference found.
 *   <dt>{@code maxOffset}
 *   <dd>Maximal offset of a back-reference.
 *   <dt>{@code maxLiteralLength}
 *   <dd>Maximal length of a literal block.
 * </dl>
 *
...
public class LZ77Compressor {
   ...
   *
   * <p>This class is not supposed to be subclassed by classes outside of Commons Compress so it is
   * considered internal and changed that would break subclasses may get introduced with future
   * releases.
   */
   ...
   *
   * <p>The callback is invoked on the same thread that receives the bytes to compress and may be
   * invoked multiple times during the execution of {@link #compress} or {@link #finish}.
   */
   ...
   *
   * <p>For performance reasons this encapsulates the real data, not a copy of it. Don't modify the
   * data and process it inside of {@link Callback#accept} immediately as it will get overwritten
   * sooner or later.
   */
  public static final class LiteralBlock extends Block {
     ...
     * The literal data.
     *
     * <p>This returns a life view of the actual data in order to avoid copying, modify the array at
     * your own risk.
     *
     * @return the data
     */
  ...
  /**
   * Feeds bytes into the compressor which in turn may emit zero or more blocks to the callback
   ...
   *
   * <p>The compressor will in turn emit at least one block ({@link EOD}) but potentially multiple
  ...
  /**
   * Inserts the current three byte sequence into the dictionary and returns the previous head of
   * the hash-chain.
   *
   * <p>Updates {@code insertHash} and {@code prev} as a side effect.
   */
  ...
  /**
   * Searches the hash chain for real matches and returns the length of the longest match (0 if none
   * were found) that isn't too far away (WRT maxOffset).
   *
   * <p>Sets matchStart to the index of the start position of the longest match as a side effect.
   */
  private int longestMatch(int matchHead) {
    ...
    final int niceBackReferenceLength =
        Math.min(maxPossibleLength, params.getNiceBackReferenceLength());
    ...
    return longestMatchLength; // < minLength if no matches have been found, will be ignored in
                               // compress()
  }
  ...
  /**
   * Assumes we are calculating the hash for three consecutive bytes as a rolling hash, i.e. for
   * bytes ABCD if H is the hash of ABC the new hash for BCD is nextHash(H, D).
   *
   * <p>The hash is shifted by five bits on each update so all effects of A have been swapped after
   * the third update.
   */
  ...
  public void prefill(final byte[] data) {
    ...
    if (len >= NUMBER_OF_BYTES_IN_HASH) {
      ...
      missedInserts = NUMBER_OF_BYTES_IN_HASH;
