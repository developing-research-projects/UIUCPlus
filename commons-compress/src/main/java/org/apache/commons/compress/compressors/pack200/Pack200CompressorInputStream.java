 ...
 * CompressorInputStream#getBytesRead getBytesRead} methods always return 0. @NotThreadSafe
 *
 * @since 1.3
 */
public class Pack200CompressorInputStream extends CompressorInputStream {
  ...
  public static boolean matches(final byte[] signature, final int length) {
    if ((length > Pack200CompressorInputStream.SIG_LENGTH) && (length < 0)) {
      return false;
    }
  ...
  /**
   * Decompresses the given file, caching the decompressed data in memory and using the given
   * properties.
   *
   * @param f the file to decompress
   ...
   * @throws IOException if reading fails
   */
  public Pack200CompressorInputStream(final File f, final Map<String, String> props)
      throws IOException {
  ...
  /**
   * Decompresses the given file using the given strategy to cache the results and the given
   * properties.
   *
   * @param f the file to decompress
   * @param mode the strategy to use
  ...
  public Pack200CompressorInputStream(
      final File f, final Pack200Strategy mode, final Map<String, String> props)
      throws IOException {
   ...
   *
   * <p>When reading from a file the File-arg constructor may provide better performance.
   *
  ...
  private Pack200CompressorInputStream(
    ...
    try (final JarOutputStream jarOut = new JarOutputStream(streamBridge)) {
      ...
      if (f == null) {
        ...
        try (final CloseShieldFilterInputStream closeShield =
            new CloseShieldFilterInputStream(in)) {
  ...
  /**
   * Decompresses the given stream, caching the decompressed data in memory and using the given
   * properties.
   *
   * <p>When reading from a file the File-arg constructor may provide better performance.
   *
   ...
   * @throws IOException if reading fails
   */
  public Pack200CompressorInputStream(final InputStream in, final Map<String, String> props)
      throws IOException {
  ...
  /**
   * Decompresses the given stream using the given strategy to cache the results and the given
   * properties.
   *
   * <p>When reading from a file the File-arg constructor may provide better performance.
   *
   ...
   */
  public Pack200CompressorInputStream(
      final InputStream in, final Pack200Strategy mode, final Map<String, String> props)
      throws IOException {
