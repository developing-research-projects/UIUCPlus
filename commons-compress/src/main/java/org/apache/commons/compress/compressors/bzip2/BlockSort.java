 ...
 *
 * <p>This class is based on a Java port of Julian Seward's blocksort.c in his libbzip2
 *
 * <p>The Burrows-Wheeler transform is a reversible transform of the original data that is supposed
 * to group similar bytes close to each other. The idea is to sort all permutations of the input and
 * only keep the last byte of each permutation. E.g. for "Commons Compress" you'd get:
 *
 * <pre>
 *  CompressCommons
 * Commons Compress
 ...
 * </pre>
 *
 * <p>Which results in a new text "ss romooCCmmpnse", in adition the index of the first line that
 * contained the original text is kept - in this case it is 1. The idea is that in a long English
 * text all permutations that start with "he" are likely suffixes of a "the" and thus they end in
 * "t" leading to a larger block of "t"s that can better be compressed by the subsequent
 * Move-to-Front, run-length und Huffman encoding steps.
 *
 * <p>For more information see for example:
 *
 * <ul>
 *   <li><a href="http://www.hpl.hp.com/techreports/Compaq-DEC/SRC-RR-124.pdf">Burrows, M. and
 *       Wheeler, D.: A Block-sorting Lossless Data Compression Algorithm</a>
 *   <li><a href="http://webglimpse.net/pubs/suffix.pdf">Manber, U. and Myers, G.: Suffix arrays: A
 *       new method for on-line string searches</a>
 *   <li><a
...
class BlockSort {
  ...
  /**
   * Array instance identical to Data's sfmap, both are used only temporarily and indepently, so we
   * do not need to allocate additional memory.
   */
  ...
  /**
   * @param fmap points to the index of the starting point of a permutation inside the block of data
   *     in the current partially sorted order
   * @param eclass points from the index of a character inside the block to the first index in fmap
   ...
   */
  private void fallbackQSort3(
      final int[] fmap, final int[] eclass, final int loSt, final int hiSt) {
   ...
   */
  private void fallbackSimpleSort(
      final int[] fmap, final int[] eclass, final int lo, final int hi) {
    if (lo == hi) {
      return;
    }
  ...
  /**
   * Adapt fallbackSort to the expected interface of the rest of the code, in particular deal with
   * the fact that block starts at offset 1 (in libbzip2 1.0.6 it starts at 0).
   */
  ...
  private void mainQSort3(
      final BZip2CompressorOutputStream.Data dataShadow,
      final int loSt,
      final int hiSt,
      final int dSt,
      final int last) {
   ...
   * unrolling has shown a noticeable performance improvement on JRE 1.4.2 (Linux i586 / HotSpot
