...
/**
 * Archive is the main entry point to pack200 and represents a packed archive. An archive is
 * constructed with either a JarInputStream and an output stream or a JarFile as input and an
 * OutputStream. Options can be set, then {@code pack()} is called, to pack the Jar file into a
 * pack200 archive.
 */
public class Archive {
   ...
   */
  public Archive(final JarFile jarFile, OutputStream outputStream, PackingOptions options)
      throws IOException {
   ...
   */
  public Archive(
      final JarInputStream inputStream, OutputStream outputStream, PackingOptions options)
      throws IOException {
  ...
  private boolean addJarEntry(
      final PackingFile packingFile,
      final List<Pack200ClassReader> javaClasses,
      final List<PackingFile> files) {
    ...
    if (segmentLimit != -1 && segmentLimit != 0) {
      ...
      currentSegmentSize += packedSize; // <--- BUGGY
  ...
  private void doNormalPack() throws IOException, Pack200Exception {
    ...
    if (jarInputStream != null) {
      packingFileList =
          PackingUtils.getPackingFileListFromJar(jarInputStream, options.isKeepFileOrder());
    } else {
