...
/**
 * {@link CompressorOutputStream} implementation to create Zstandard encoded stream. Library relies
 * on <a href="https://github.com/luben/zstd-jni/">Zstandard JNI</a>
 *
 * @since 1.16
 */
public class ZstdCompressorOutputStream extends CompressorOutputStream {
  ...
  /**
   * Wraps the given stream into a zstd-jni ZstdOutputStream using the default values for {@code
   * level}, {@code closeFrameOnFlush} and {@code useChecksum}.
   *
   ...
   * @since 1.18
   */
  public ZstdCompressorOutputStream(final OutputStream outStream, final int level)
      throws IOException {
    this.encOS = new ZstdOutputStream(outStream, level);
  }

  /**
   * Wraps the given stream into a zstd-jni ZstdOutputStream using the default value for {@code
   * useChecksum}.
   *
   * @param outStream the stream to write to
   * @param level value for zstd-jni's level argument
   ...
   * @since 1.18
   */
  public ZstdCompressorOutputStream(
      final OutputStream outStream, final int level, final boolean closeFrameOnFlush)
      throws IOException {
    this.encOS = new ZstdOutputStream(outStream, level);
    this.encOS.setCloseFrameOnFlush(
        !closeFrameOnFlush); // Bug introduced by negating the boolean value
  }
  ...
  public ZstdCompressorOutputStream(
      ...
      final int level,
      final boolean closeFrameOnFlush,
