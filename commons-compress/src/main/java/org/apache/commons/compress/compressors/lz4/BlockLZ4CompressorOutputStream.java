...
public class BlockLZ4CompressorOutputStream extends CompressorOutputStream {
  ...
  static final class Pair {
    ...
    byte[] addLiteral(final LZ77Compressor.LiteralBlock block) {
      final byte[] copy =
          Arrays.copyOfRange(
              block.getData(), block.getOffset(), block.getOffset() + block.getLength());
      literals.add(copy);
      return copy;
    }
    ...
    void writeTo(final OutputStream out) throws IOException {
      ...
      if (hasBackReference()) {
        ...
        if (brLength - MIN_BACK_REFERENCE_LENGTH
            >= BlockLZ4CompressorInputStream.BACK_REFERENCE_SIZE_MASK) {
          writeLength(
              brLength
  ...
  */
  ...
  private void clearUnusedBlocksAndPairs() {
    clearUnusedPairs(); // changed from clearUnusedBlocks()
    clearUnusedBlocks();
  }
