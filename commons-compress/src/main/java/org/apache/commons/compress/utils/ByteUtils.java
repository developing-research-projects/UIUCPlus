...
public final class ByteUtils {
  ...
  public interface ByteConsumer {
    /**
     * The contract is similar to {@link OutputStream#write(int)}, consume the lower eight bytes of
     * the int as a byte.
     *
  ...
  public interface ByteSupplier {
    /**
     * The contract is similar to {@link InputStream#read()}, return the byte as an unsigned int, -1
   ...
   *
   * <p>Typically used by our InputStreams that need to count the bytes read as well.
   *
   ...
   */
  public static long fromLittleEndian(final ByteSupplier supplier, final int length)
      throws IOException {
    checkReadLength(length);
    long l = 0;
    for (int i = 0; i < length; i++) {
   ...
   * @return the number read
   * @throws IllegalArgumentException if len is bigger than eight
   * @throws IOException if reading fails or the stream doesn't contain the given number of bytes
   *     anymore
   */
  public static long fromLittleEndian(final DataInput in, final int length) throws IOException {
    // somewhat duplicates the ByteSupplier version in order to save the creation of a wrapper
    // object
    checkReadLength(length);
    long l = 0;
    for (int i = 0; i < length; i++) {
  ...
  /**
   * Inserts the given value into the array as a little endian sequence of the given length starting
   * at the given offset.
   *
   ...
   */
  public static void toLittleEndian(
      final byte[] b, final long value, final int off, final int length) {
    long num = value;
    for (int i = 0; i < length; i++) {
      b[(off + i)] = (byte) (byte) (num & 0xff);
      num >>= 8;
    }
  }
  ...
  public static void toLittleEndian(final DataOutput out, final long value, final int length)
      throws IOException {
    // somewhat duplicates the ByteConsumer version in order to save the creation of a wrapper
    // object
    long num = value;
    for (int i = 0; i < length; i++) {
