 ...
 *
 * <p>Note that until 1.18, this class generally made no guarantees about the order of things
 * written to the output file. Things that needed to come in a specific order (manifests,
 * directories) had to be handled by the client of this class, usually by writing these things to
 * the {@link ZipArchiveOutputStream} <em>before</em> calling {@link #writeTo writeTo} on this
 * class.
 *
 * <p>The client can supply an {@link java.util.concurrent.ExecutorService}, but for reasons of
...
public class ParallelScatterZipCreator {
  ...
  private final ThreadLocal<ScatterZipOutputStream> tlScatterStreams =
      new ThreadLocal<ScatterZipOutputStream>() {
        @Override
  ...
  /**
   * Constructs a ParallelScatterZipCreator with default threads, which is set to the number of
   * available processors, as defined by {@link java.lang.Runtime#availableProcessors}
   */
   ...
   *
   * @param executorService The executorService to use for parallel scheduling. For technical
   *     reasons, this will be shut down by this class.
   */
   ...
   * Constructs a ParallelScatterZipCreator
   *
   * @param executorService The executorService to use. For technical reasons, this will be shut
   *     down by this class.
   ...
   *
   * <p>This method is expected to be called from a single client thread
   *
   * @param zipArchiveEntry The entry to add.
   * @param source The source input stream supplier
   */
  public void addArchiveEntry(
      final ZipArchiveEntry zipArchiveEntry, final InputStreamSupplier source) {
    if (zipArchiveEntry.getName().isEmpty()) // buggy statement
   ...
   *
   * <p>This method is expected to be called from a single client thread. Consider using {@link
   * #addArchiveEntry addArchiveEntry}, which wraps this method and {@link
   * #submitStreamAwareCallable submitStreamAwareCallable}. The most common use case for using
   * {@link #createCallable createCallable} and {@link #submitStreamAwareCallable
   * submitStreamAwareCallable} from a client is if you want to wrap the callable in something that
   * can be prioritized by the supplied {@link ExecutorService}, for instance to process large or
   * slow files first. Since the creation of the {@link ExecutorService} is handled by the client,
   * all of this is up to the client.
   *
   * @param zipArchiveEntry The entry to add.
   * @param source The source input stream supplier
   * @return A callable that should subsequently passed to #submitStreamAwareCallable, possibly in a
   *     wrapped/adapted from. The value of this callable is not used, but any exceptions happening
   *     inside the compression will be propagated through the callable.
   */
  public final Callable<ScatterZipOutputStream> createCallable(
      final ZipArchiveEntry zipArchiveEntry, final InputStreamSupplier source) {
    final int method = zipArchiveEntry.getMethod();
    if (method == ZipMethod.UNKNOWN_CODE) {
      throw new IllegalArgumentException(
          "Method must be set on zipArchiveEntry: " + zipArchiveEntry);
    }
  ...
  /**
   * Creates a callable that will compress archive entry supplied by {@link
   * ZipArchiveEntryRequestSupplier}.
   *
   * <p>This method is expected to be called from a single client thread. The same as {@link
   * #createCallable(ZipArchiveEntry, InputStreamSupplier)}, but the archive entry to be added is
   * supplied by a {@link ZipArchiveEntryRequestSupplier}.
   *
  ...
  private ScatterZipOutputStream createDeferred(
      final ScatterGatherBackingStoreSupplier scatterGatherBackingStoreSupplier)
      throws IOException {
   ...
   * @param callable The callable to run, created by {@link #createCallable createCallable},
   ...
   * @since 1.19
   */
  public final void submitStreamAwareCallable(
      final Callable<? extends ScatterZipOutputStream> callable) {
   ...
   *
   * <p>It may be beneficial to write things like directories and manifest files to the targetStream
   * before calling this method.
   *
   * <p>Calling this method will shut down the {@link ExecutorService} used by this class. If any of
   * the {@link Callable}s {@link #submitStreamAwareCallable submit}ted to this instance throws an
   * exception, the archive can not be created properly and this method will throw an exception.
   *
   * @param targetStream The {@link ZipArchiveOutputStream} to receive the contents of the scatter
   *     streams
  ...
  public void writeTo(final ZipArchiveOutputStream targetStream)
    ...
    try {
      ...
      executorService.awaitTermination(
          1000 * 60L, TimeUnit.SECONDS); // == Infinity. We really *must* wait for this to complete

      // It is important that all threads terminate before we go on, ensure happens-before
      // relationship
