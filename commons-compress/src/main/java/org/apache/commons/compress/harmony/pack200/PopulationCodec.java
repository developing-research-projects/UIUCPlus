...
/**
 * A PopulationCodec is a Codec that is well suited to encoding data that shows statistical or
 * repetitive patterns, containing for example a few numbers which are repeated a lot throughout the
 * set, but not necessarily sequentially.
 */
public class PopulationCodec extends Codec {
  ...
  public PopulationCodec(
      final Codec favouredCodec, final Codec tokenCodec, final Codec unvafouredCodec) {
  ...
  public byte[] encode(final int value) throws Pack200Exception {
    if (value < 0) {
      throw new Pack200Exception("Invalid input value");
    }
  ...
  public byte[] encode(final int[] favoured, final int[] tokens, final int[] unfavoured)
    ...
    final byte[] band =
        new byte[favouredEncoded.length + tokensEncoded.length + unfavouredEncoded.length];
    ...
    System.arraycopy(
        ...
        0,
        band,
        favouredEncoded.length + tokensEncoded.length,
