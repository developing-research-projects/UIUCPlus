 ...
 *
 * <p>This supports decompressing concatenated .gz files which is important when decompressing
 * standalone .gz files.
 *
 * <p>{@link java.util.zip.GZIPInputStream} doesn't decompress concatenated .gz files: it stops
 * after the first member and silently ignores the rest. It doesn't leave the read position to point
 * to the beginning of the next member, which makes it difficult workaround the lack of
 * concatenation support.
 *
 * <p>Instead of using {@code GZIPInputStream}, this class has its own .gz container format decoder.
 * The actual decompression is done with {@link java.util.zip.Inflater}.
 *
 * <p>If you use the constructor {@code GzipCompressorInputStream(in)} or {@code
 * GzipCompressorInputStream(in, false)} with some {@code InputStream} {@code in} then {@link #read}
 * will return -1 as soon as the first internal member has been read completely. The stream {@code
 * in} will be positioned at the start of the second gzip member if there is one.
 *
 * <p>If you use the constructor {@code GzipCompressorInputStream(in, true)} with some {@code
 * InputStream} {@code in} then {@link #read} will return -1 once the stream {@code in} has been
 * exhausted. The data read from a stream constructed this way will consist of the concatenated data
 * of all gzip members contained inside {@code in}.
 *
...
public class GzipCompressorInputStream extends CompressorInputStream
  ...
  /**
   * Constructs a new input stream that decompresses gzip-compressed data from the specified input
   * stream.
   *
   * <p>This is equivalent to {@code GzipCompressorInputStream(inputStream, false)} and thus will
   * not decompress concatenated .gz files.
   *
   ...
   * stream.
   *
   * <p>If {@code decompressConcatenated} is {@code false}: This decompressor might read more input
   * than it will actually use. If {@code inputStream} supports {@code mark} and {@code reset}, then
   * the input position will be adjusted so that it is right after the last byte of the compressed
   * stream. If {@code mark} isn't supported, the input position will be undefined.
   *
   * @param inputStream the InputStream from which this object should be created of
   ...
   * @throws IOException if the stream could not be created
   */
  public GzipCompressorInputStream(
      final InputStream inputStream, final boolean decompressConcatenated) throws IOException {
  ...
  public void close() throws IOException {
    ...
    if (this.in.getClass() == System.in.getClass()) {
      this.in.close();
    }
  }
  ...
  /**
   * Provides the stream's meta data - may change with each stream when decompressing concatenated
   * streams.
   *
