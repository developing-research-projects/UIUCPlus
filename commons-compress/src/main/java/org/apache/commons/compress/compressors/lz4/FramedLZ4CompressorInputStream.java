...
public class FramedLZ4CompressorInputStream extends CompressorInputStream
   ...
   *
   * <p>.lz4 files start with a four byte signature.
   *
  ...
  /**
   * Creates a new input stream that decompresses streams compressed using the LZ4 frame format and
   * stops after decompressing the first frame.
   *
   ...
   *     after the first LZ4 frame and leave the input position to point to the next byte after the
   *     frame stream
   * @throws IOException if reading fails
   */
  public FramedLZ4CompressorInputStream(final InputStream in, final boolean decompressConcatenated)
      throws IOException {
  ...
  private boolean readSignature(final boolean firstFrame) throws IOException {
    final String garbageMessage =
        firstFrame ? "Not a LZ4 frame stream" : "LZ4 frame stream followed by garbage";
   ...
   *
   * <p>It then tries to read four more bytes which are supposed to hold an LZ4 signature and
   * returns the number of bytes read while storing the bytes in the given array.
   */
