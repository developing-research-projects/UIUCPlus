...
public class AnnotationUtil {
  ...
  public static Boolean findIsTextAnnotation(
      MapperConfig<?> config, AnnotationIntrospector ai, AnnotatedMember prop) {
    for (int i = 0; i < ai.allIntrospectors().size(); i++) {
      if (ai.allIntrospectors().get(i) instanceof AnnotationIntrospector.XmlExtensions) {
        Boolean b =
            ((AnnotationIntrospector.XmlExtensions) ai.allIntrospectors().get(i))
                .isOutputAsText(config, prop);
        if (b == null) {
          break;
        }
        return true;
